...
#Region "Genera Reportes"
    ...
    Private Function FblnGeneroReporte2() As Boolean
        ...
        Select Case EnuReporte
            ...
            Case EnuReporteDef.enuAnticiposPorPredioAgru
                SGenereAnticiposPorPredioAgru()
            Case Else
                lblnGenero = False
        End Select
        Return lblnGenero
    ...
    Friend Function SGenereItemsProgFactXPreAgr() As String
        Dim lstrMens = String.Empty
        GobjPanDat.SControleProcesoObj(True)
        Dim lfrmRep = FfrmRep()
        With lfrmRep
            .RcReporte = New repItemsProgFactXPreAgr
            Using ldsItemsProgFact As New DataSet
                SGenereDataSetItemsProgFactXPreAgr(ldsItemsProgFact)
                Dim ldtbItemsProgFact = ldsItemsProgFact.Tables("OriItemsProgFact")
                If ldtbItemsProgFact.Rows.Count = 0 Then
                    lstrMens = "No hay información en la fecha señalada!"
                Else
    ...
    Private Sub SGenereAnticiposPorPredioAgru()
        GobjPanDat.SControleProcesoObj(True)
        Dim lfrmRep = FfrmRep()
        With lfrmRep
            .RcReporte = New repAntPorPredioAgr
            Using ldsAntPorPreAgr As New DataSet
                SGenereAntPorPredioAgru(ldsAntPorPreAgr, Now)
                .RcReporte.SetDataSource(ldsAntPorPreAgr)
                .Show()
            End Using
        End With
        GobjPanDat.SControleProcesoObj(False)
    ...
    Friend Sub SGenereRepFacturasMultadas(adtmFechaDatos As Date)
        GobjPanDat.SControleProcesoObj(True)
        Dim lfrmRep = FfrmRep()
        With lfrmRep
            .RcReporte = New repFacturasMultadas
            Using ldsMultas As New DataSet
                SGenereDataSetFacsMultadas(ldsMultas)
                .RcReporte.SetDataSource(ldsMultas)
                SExporteMultasAplicadas(.RcReporte, adtmFechaDatos)
                .Show()
            End Using
        End With
...
#Region "Exportar reportes"
    ...
    Friend Sub SExporteFactura(aobjFactura As ClsFactura, aobjTerCentroUtil As ClsTercero,
        ...
        Try
            ...
            Else
                ...
                ElseIf GobjParametros.ObjAnoActual.ObjTipoIncentivoByt.ObjValorPro =
                        EnuTipoIncentivo.EnuDescuentoPP Then
                    lrcFactura = New repFacturaDsctoPPLogo
                Else
                    lrcFactura = New repFacturaLogo
    ...
    Private Sub SExporteMultasAplicadas(arcMultasAplicadas As ReportClass,
                adtmFechaDatos As Date)
        Dim leopOpcionesExp As New ExportOptions
        Dim lddoOpcionesDisco As DiskFileDestinationOptions =
                ExportOptions.CreateDiskFileDestinationOptions()
        lddoOpcionesDisco.DiskFileName = FstrNombreArchMultasAplicadas(EnuTipoArchivoDef.EnuPdf,
                adtmFechaDatos)
        leopOpcionesExp.ExportFormatType = ExportFormatType.PortableDocFormat
        leopOpcionesExp.ExportDestinationType = ExportDestinationType.DiskFile
        leopOpcionesExp.ExportDestinationOptions = lddoOpcionesDisco
        arcMultasAplicadas.Export(leopOpcionesExp)
        '
        lddoOpcionesDisco.DiskFileName = FstrNombreArchMultasAplicadas(
                EnuTipoArchivoDef.EnuCrystalRepo, adtmFechaDatos)
        leopOpcionesExp.ExportFormatType = ExportFormatType.CrystalReport
        leopOpcionesExp.ExportDestinationOptions = lddoOpcionesDisco
        arcMultasAplicadas.Export(leopOpcionesExp)
    ...
    Private Function FstrNombreArchMultasAplicadas(aenuTipoArchivo As EnuTipoArchivoDef,
                adtmFechaDatos As Date) As String
        Dim lstrExtension = String.Empty
        Select Case aenuTipoArchivo
            Case EnuTipoArchivoDef.EnuPdf
        ...
        End If
        With MstbExpresionSql
            .Clear.Append(lstrTrayFM).Append("\MultasAplicadas_FM_")
            .Append(adtmFechaDatos.Year.ToString).Append(Format(adtmFechaDatos.Month, "0#"))
            .Append(Format(adtmFechaDatos.Day, "0#")).Append("_")
        End With
        Dim lstrNomArchivoBase = MstbExpresionSql.ToString
...
#Region "Factura"
    ...
    Private Function FstrExpSqlFactura(astrPrefFacturas As String, astrIdFacIni As String,
        ...
        With MstbExpresionSql
            ...
            .Append(ClsPieFacturaTres_FactStr.SstrNombreCampoBd).Append(", ")
    ...
    Private Function FstrExpDetEstadoCuenta(astrPrefFacturas As String, astrIdFacIni As String,
        ...
        With MstbExpresionSql
            ...
            .Append("SELECT ").Append(ClsPrefijoFac_EstadoStr.SstrNombreCampoBd).Append(", ")
            .Append(ClsIdFactura_EstadoEnt.SstrNombreCampoBd).Append(", IF(")
            .Append(ClsIdAno_ItemFactEstadoShr.SstrNombreCampoBd).Append(" > 0 AND ")
            .Append(ClsIdServicioItemFac_EstadoShr.SstrNombreCampoBd).Append(" = 1, ")
            .Append(lshrIdAnoAct).Append(", ")
            .Append(ClsIdAno_ItemFactEstadoShr.SstrNombreCampoBd)
            .Append(") AS IdAnos").Append(", ")
            .Append(ClsIdServicioItemFac_EstadoShr.SstrNombreCampoBd)
            .Append(", SUM(F.").Append(ClsDeudaCap_ItFacEstDec.SstrNombreCampoBd)
            .Append(") AS DeudaCap, ")
            .Append("IF(E.").Append(ClsDeudaIntMoraDec.SstrNombreCampoBd)
            .Append(" > 0, SUM(F.").Append(ClsDeudaIntMes_ItFacEstDec.SstrNombreCampoBd)
            .Append("), 0) AS IntMes, ")
            .Append("IF(E.").Append(ClsDeudaIntMoraDec.SstrNombreCampoBd)
            .Append(" > 0, SUM(F.").Append(ClsDeudaIntMora_ItFacEstDec.SstrNombreCampoBd)
            .Append(" - ").Append(ClsDeudaIntMes_ItFacEstDec.SstrNombreCampoBd)
            .Append("), 0) AS ValorInt ").Append("FROM ")
            .Append(ClsFacturaEstado.SstrNombreTabla)
            .Append(" AS F INNER JOIN ").Append(ClsEstadoCuenta.SstrNombreTabla)
            .Append(" AS E ON F.").Append(MstrCampoBdCarpeta).Append(" = E.")
            .Append(MstrCampoBdCarpeta).Append(" AND F.").Append(MstrCampoBdCentroUtil)
            .Append(" = E.").Append(MstrCampoBdCentroUtil).Append(" AND F.")
            .Append(ClsIdEstado_FactEstadoEnt.SstrNombreCampoBd).Append(" = E.")
            .Append(ClsIdEstadoCuentaEnt.SstrNombreCampoBd).Append(" WHERE F.")
            .Append(MstrCampoBdCarpeta).Append(" = ").Append(GshrIdCarpeta.ToString)
            .Append(" AND F.").Append(MstrCampoBdCentroUtil).Append(" = ")
            .Append(GshrIdCentroUtil.ToString).Append(" AND ")
    ...
    Private Sub SComplementeDtbFacturas(adsDataSetFacturas As DataSet)
        ...
        If Not IsNothing(adsDataSetFacturas) Then
            ...
            For Each ldrwFra As DataRow In ldtbFacturas.Rows
                ...
                ldecValorIva = lobjFra.DecIvaFactura
                ldecValorBaseIva = lobjFra.DecBaseIvaFactura
...
#Region "Notas Crédito"
    Private Sub SGenereDataSetNotasCr(adsNotasCr As DataSet)
        ...
        With lstbExpresion
            ...
            .Append(ClsIdTipoDscto_ItemNotaCrByt.SstrNombreCampoBd)
            .Append(", '' AS Detalle, '' as TipoDscto,")
            .Append(ClsValor_ItemNotaCrDec.SstrNombreCampoBd)
            .Append(" FROM ").Append(ClsItemNotaCr.SstrNombreTabla)
            .Append(" WHERE ").Append(MstrCampoBdCarpeta).Append(" = ")
            .Append(GshrIdCarpeta.ToString)
            .Append(" AND ").Append(MstrCampoBdCentroUtil).Append(" = ")
            .Append(GshrIdCentroUtil.ToString)
            .Append(" AND ").Append(ClsPrefijo_NotaCrStr.SstrNombreCampoBd).Append(" = '")
            .Append(lstrPrefNotaCr).Append("' AND ")
            .Append(ClsIdNotaCr_ItemNotaCrEnt.SstrNombreCampoBd)
            .Append(" >= ").Append(lstrIdNotaCrIni).Append(" AND ")
            .Append(ClsIdNotaCr_ItemNotaCrEnt.SstrNombreCampoBd).Append(" <= ")
            .Append(lstrIdNotaCrFin).Append(" ORDER BY ")
            .Append(ClsPrefijoFact_ItemNotaCrStr.SstrNombreCampoBd).Append(CLSTRCOMA)
    ...
    Private Shared Sub SComplementeDsNotasCr(adsNotasCr As DataSet)
        If Not IsNothing(adsNotasCr) Then
            ...
            For Each ldrwItem As DataRow In ldtbItemsNotaCr.Rows
                Dim lenuTipoDescuento As EnuTipoDescuento = ClsPanorama.FobjValorCampo(ldrwItem(
                ...
                End Select
                ldrwItem("TipoDscto") = lstrDetalle
...
#Region "Predios"
    ...
    Private Function FstrExpItemsProgFact() As String
        With MstbExpresionSql
            ...
            .Append(ClsIdAno_ItemProgramaFactShr.SstrNombreCampoBd)
    ...
    Private Sub SComplementeDtbItemsProgFact(adsItesmProgFact As DataSet)
        ...
        Next
        Dim ldtbCenUtil = adsItesmProgFact.Tables(ClsCentroUtilidad.SstrNombreTabla)
        ldtbCenUtil.Rows(0)("Valor") = lobjServicio.FdecValor
    End Sub

    ' Items Programa Facturación por Predio Agrupador
    Private Sub SGenereDataSetItemsProgFactXPreAgr(adsItemsProgFact As DataSet)
        Dim lstrSqlItemsProgFact = FstrExpItemsProgFact()
        Dim lstrSqlPrediosAgrupadores = FstrExpSqlPrediosAgrupadores()
        Dim lcolExpresionesSql As New Collection
        Dim lcolNombresTablas As New Collection
        lcolExpresionesSql.Add(lstrSqlItemsProgFact)
        lcolExpresionesSql.Add(lstrSqlPrediosAgrupadores)
        lcolNombresTablas.Add("OriItemsProgFact")
        lcolNombresTablas.Add("OriPredios")
        GobjPanDat.SdsDataSet(adsItemsProgFact, lcolExpresionesSql, lcolNombresTablas)
        adsItemsProgFact.Tables.Add(FdtbCentroUtilidad)
        SComplementeDtbItemsProgFact(adsItemsProgFact)
        Dim lstrNomArch = GstrTrayDatPrg & "ItemsProgFactXPreAgr" & ".XML"
        adsItemsProgFact.WriteXml(lstrNomArch, System.Data.XmlWriteMode.WriteSchema)
        adsItemsProgFact.Dispose()
    End Sub

    Private Function FstrExpSqlPrediosAgrupadores() As String
        With MstbExpresionSql
            .Clear()
            .Append("SELECT ").Append(ClsIdPredioAgrupadorStr.SstrNombreCampoBd).Append(", ")
            .Append(ClsIdPredioStr.SstrNombreCampoBd).Append(" FROM ")
            .Append(ClsPredio.SstrNombreTabla).Append(" WHERE ")
            .Append(PanL.ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCarpeta)
            .Append(" AND ").Append(PanL.ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCentroUtil).Append(" ORDER BY ")
...
#Region "Recibos de Caja entre Fechas"
    Private Sub SGenereDataSetRCFechas(adsRCFechas As DataSet)
        ...
        Dim lstrSqlNotasRevRC = FstrExpNotasRevRC()
        Dim lcolExpresionesSql As New Collection
        Dim lcolNombresTablas As New Collection
        lcolExpresionesSql.Add(lstrSqlRCFechas)
        lcolExpresionesSql.Add(lstrSqlNotasRevRC)
        lcolNombresTablas.Add("OriRecibosCaja")
        lcolNombresTablas.Add("OriNotasRevRC")
    ...
    Private Function FstrExpNotasRevRC() As String
        Dim lstrTabla = ClsNotaReversionCr.SstrNombreTabla
        Dim lstrCamposSelect = {ClsPrefijoDoc_NotaReversaCrStr.SstrNombreCampoBd,
                ClsIdDoc_NotaReversaCrEnt.SstrNombreCampoBd}
        Dim lstrIndice = {{ClsPrefijoDoc_NotaReversaCrStr.SstrNombreCampoBd, "ASC"},
                {ClsIdDoc_NotaReversaCrEnt.SstrNombreCampoBd, "ASC"}}
        Dim lstrFiltro As String = ClsOrionCop.StrFiltroUbicacion & " AND " &
                ClsFechaCreacionDtm.SstrNombreCampoBd &
                " > '" & StrFechaDesde & "' AND " & ClsFechaCreacionDtm.SstrNombreCampoBd &
                " < '" & StrFechaHasta & "' AND " & ClsTipoDocReversadoByt.SstrNombreCampoBd &
                " = " & EnuDocReversado.EnuReciboC
        Dim lstrSql = ClsPanoramaDat.FstrConstruyaExpSqlSelect(lstrTabla, lstrCamposSelect,
                lstrIndice, lstrFiltro, Array.Empty(Of String)())
        Return lstrSql
    End Function
    ...
    Private Shared Sub SComplementeDtbRCFechas(adsRCFechas As DataSet)
        ...
        Next
        Dim ldtbNotasRevRC As DataTable =
                adsRCFechas.Tables("OriNotasRevRC")
        Dim lstrFiltro As String
        Dim ldrwRecCaja As DataRow()
        For Each ldrwNotasRev As DataRow In ldtbNotasRevRC.Rows
            lstrPref = ClsPanorama.FobjValorCampo(ldrwNotasRev(
                    ClsPrefijoDoc_NotaReversaCrStr.SstrNombreCampoBd), EnuTipoValor.EnuString)
            lentIdRC = ClsPanorama.FobjValorCampo(ldrwNotasRev(
                    ClsIdDoc_NotaReversaCrEnt.SstrNombreCampoBd), EnuTipoValor.EnuInteger)
            lstrFiltro = ClsPrefijo_RecStr.SstrNombreCampoBd & " = '" &
                    lstrPref & "' AND " & ClsIdRecCajaEnt.SstrNombreCampoBd & " = " & lentIdRC
            If ldtbRCFechas.Select(lstrFiltro).Length > 0 Then
                ldrwRecCaja = ldtbRCFechas.Select(lstrFiltro)
                ldrwRecCaja(0)("RC_Anulado") = "Si"
            End If
        Next
    End Sub
#End Region
...
#Region "Clientes"
    ...
    Private Function FstrExpCarteraPorCliente() As String 'Ok TipoNov
        ...
        With MstbExpresionSql
            ...
            .Append(lstrFiltroUbicacion).Append(" AND ").Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd)
            .Append(" <> '***' AND ").Append(ClsFechaNovedadDtm.SstrNombreCampoBd).Append(" <= '")
            .Append(StrFechaHasta).Append("' GROUP BY ").Append(ClsIdTercero_NovDbl.SstrNombreCampoBd)
            .Append(", ").Append(ClsIdPredioAgrupador_NovStr.SstrNombreCampoBd).Append(", ")
            .Append("Fecha").Append(" ORDER BY ").Append(ClsIdPredioAgrupador_NovStr.SstrNombreCampoBd)
        End With
        Return MstbExpresionSql.ToString
    End Function
    ...
    Private Function FstrExpCarteraPorPredioAgr() As String
        With MstbExpresionSql
            ...
            .Append(" <> '' AND ").Append(ClsPrefijo_FactStr.SstrNombreCampoBd)
            .Append(" <> '***' GROUP BY ")
            .Append(ClsIdPredioAgrupador_FacStr.SstrNombreCampoBd).Append(", ")
    ...
    Private Function FstrExpCarteraPorPredio() As String
        With MstbExpresionSql
            ...
            .Append(ClsCreditos_ItemFactDec.SstrNombreCampoBd).Append(" AND I.")
            .Append(ClsPrefijo_ItemFactStr.SstrNombreCampoBd).Append(" <> '***'").Append(" GROUP BY ")
    ...
    Private Function FstrExpCarteraPorServicio() As String
        ...
        Dim lstrTablaSec = ClsServicio.SstrNombreTabla
        Dim lstrCamposSec As String() = {ClsConceptoServicioStr.SstrNombreCampoBd & " AS " &
        ...
        With MstbExpresionSql
            ...
            .Append(" <> ").Append(ClsCreditos_ItemFactDec.SstrNombreCampoBd).Append(" AND ")
            .Append(ClsPrefijo_FactStr.SstrNombreCampoBd).Append(" <> '***'")
        End With
    ...
    Private Function FstrExpEdadCartera(aentLimite1 As Integer, aentLimite2 As Integer,
        ...
        With MstbExpresionSql
            ...
            .Append(" WHERE ").Append(lstrFiltroUbicacion).Append(" AND ")
            .Append(ClsDebitos_FactDec.SstrNombreCampoBd).Append(" <> ")
            .Append(ClsCreditos_FactDec.SstrNombreCampoBd).Append(" AND ")
            .Append(ClsPrefijo_FactStr.SstrNombreCampoBd).Append(" <> '***'")
    ...
    ' Facturas multadas
    Private Sub SGenereDataSetFacsMultadas(adsFacsMultadas As DataSet)
        Dim lstrSqlFacturas = FstrSqlFacsMultadas()
        ...
        lcolNombresTablas.Add("OriClientes")
        GobjPanDat.SdsDataSet(adsFacsMultadas, lcolExpresionesSql, lcolNombresTablas)
        adsFacsMultadas.Tables.Add(FdtbCentroUtilidad)
        'Dim lstrNomArch = GstrTrayDatPrg & "FacturasMultadas" & ".XML"
        'adsFacsMultadas.WriteXml(lstrNomArch, System.Data.XmlWriteMode.WriteSchema)
    End Sub

    Private Function FstrSqlFacsMultadas() As String
        Dim ldtmFechaMulta =
                GobjParametros.ObjAnoActual.ObjPeriodoActual.DtmFechaFinPeriodo.AddDays(1)
        Dim lstrFechaMul = "'" & ClsPanoramaDat.FstrFechaNormalizada(ldtmFechaMulta) & "'"
        Dim lstrTabla = ClsFactura.SstrNombreTabla
        Dim lstrCampSel As String() = {ClsPrefijo_FactStr.SstrNombreCampoBd,
                ClsIdFacturaEnt.SstrNombreCampoBd, ClsIdCliente_FactDbl.SstrNombreCampoBd,
                ClsIdPredioAgrupador_FacStr.SstrNombreCampoBd,
                ClsFechaVencimientoDtm.SstrNombreCampoBd, ClsFechaMultaDtm.SstrNombreCampoBd,
                ClsDebitos_FactDec.SstrNombreCampoBd &
                " - " & ClsCreditos_FactDec.SstrNombreCampoBd & " AS Deuda"}
        Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion & " AND " &
                ClsFechaMultaDtm.SstrNombreCampoBd & " = " & lstrFechaMul
        Dim lstrOrden As String(,) = {{ClsIdPredioAgrupador_FacStr.SstrNombreCampoBd, "ASC"}}
        Dim lstrSql = ClsPanoramaDat.FstrConstruyaExpSqlSelect(lstrTabla, lstrCampSel, lstrOrden,
                lstrFiltro, {})
        Return lstrSql
    End Function
    ...
    ' Anticipos por predio agrupador
    Private Sub SGenereAntPorPredioAgru(adsAntPorPredioAgr As DataSet, adtmFechaRep As Date)
        Dim lstrSqlAntPorAplicar = FstrExpAntPorAplicar()
        Dim lcolExpresionesSql As New Collection
        Dim lcolNombresTablas As New Collection
        lcolExpresionesSql.Add(lstrSqlAntPorAplicar)
        lcolNombresTablas.Add("OriAntPorAplicar")
        GobjPanDat.SdsDataSet(adsAntPorPredioAgr, lcolExpresionesSql, lcolNombresTablas)
        adsAntPorPredioAgr.Tables.Add(FdtbCentroUtilidad)
        SComplementeTablaAnt(adsAntPorPredioAgr, adtmFechaRep)
        'Dim lstrNomArch = GstrTrayDatPrg & "AntPorAplicar" & ".XML"
        'adsAntPorPredioAgr.WriteXml(lstrNomArch, System.Data.XmlWriteMode.WriteSchema)
    End Sub
...
#Region "Auxiliar Contable"
    ...
    Private Function FstrSqlClientes() As String
        ...
        Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion
        Dim lstrCamposSelect = {ClsIdClienteDbl.SstrNombreCampoBd,
                ClsNombreCompletoStr.SstrNombreCampoBd, "'' AS PredioAgrupador",
                "0 as IntPorCausar", "'' AS AgrupadorServicios"}
        Dim lstrNombreTabla = ClsCliente.SstrNombreTabla
        Dim lstrIndice = {{ClsIdClienteDbl.SstrNombreCampoBd, "ASC"}}
        Dim lstrSql = ClsPanoramaDat.FstrConstruyaExpSqlSelect(lstrNombreTabla, lstrCamposSelect,
                lstrIndice, lstrFiltro, Array.Empty(Of String)())
...
#Region "EFactura"
    Private Sub SGenereDataSetEFacNoReg(adsDocNoRegEFac As DataSet)
        ...
        ' Orden
        With MstbExpresionSql
            .Append(" ORDER BY " & ClsIdEstadoEDocEnt.SstrNombreCampoBd &
                    ", Doc, Prefijo, IdDocumento")
        End With
        Dim lstrOrden = MstbExpresionSql.ToString
        MstbExpresionSql.Clear()
        ' Consulta Total
    ...
    Private Shared Function FstrExpSqlFacNoReg() As String
        ...
        Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion & " AND " &
                ClsIdEstadoEDocEnt.SstrNombreCampoBd & " <> " & EnuEstadoEDoc.EnuEnviada &
                 " AND " & ClsIdEstadoEDocEnt.SstrNombreCampoBd & " <> " &
                EnuEstadoEDoc.EnuNoEDoc
        Dim lstrSql = ClsPanoramaDat.FstrConstruyaExpSqlSelect(lstrNombreTabla, lstrCamposSelect,
                lstrIndice, lstrFiltro, Array.Empty(Of String)())
        Return lstrSql
    ...
    Private Shared Function FstrExpSqlNcrNoReg() As String
        ...
        Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion & " AND " &
                ClsIdEstadoEDocEnt.SstrNombreCampoBd & " <> " & EnuEstadoEDoc.EnuEnviada &
                " AND " & ClsIdTipoNotaCrByt.SstrNombreCampoBd & " <> " &
                EnuTipoNotaCrDef.EnuRetenciones & " AND " &
                ClsIdEstadoEDocEnt.SstrNombreCampoBd & " <> " & EnuEstadoEDoc.EnuNoEDoc
        Dim lstrIndice(,) = {{"", ""}}
        Dim lstrSql = ClsPanoramaDat.FstrConstruyaExpSqlSelect(lstrTabla, lstrCamposSelect,
                lstrIndice, lstrFiltro, Array.Empty(Of String))
        Return lstrSql
...
#Region "Copropiedad"
    Private Function FdtbCentroUtilidad() As DataTable
        ...
        Dim ldclValor As New DataColumn("Valor", Type.GetType("System.Decimal"))
        ...
        ldtbCentroUtilidad.Columns.AddRange({ldclIdCentrUtil, ldclNomCenUtil, ldclDireccion1,
                ldclDireccion2, ldclFechaDatos, ldclFechaDesde, ldclFechaHasta, ldclLogo,
                ldclFirma, ldclCodigoQR, ldclNombreReLeg, ldclCargo, ldclTel1, ldclTel2,
                ldclPagWEb, ldclIdRepLegal, ldclTipoDocId, ldclTelRepLeg, ldclValor})
