...
#Region "Invalida metodos en la clase base que implementan la Interfaz"
    Protected Overrides Sub SLoad()
        ...
        SHabiliteMenuEFac()
        SMuestreFacturas(Not BlnVentanaAux)
    End Sub
    ...
    Protected Overrides Sub SInicialiceControles()
        ...
        If GobjParametros.BlnEFacAutorizado Then
            lblLista.Content = "Lista Facturas"
        End If
        lblLista.Content &= " (últimos dos meses)"
        '
        ...
        If EnuOperacionEnWin = EnuOperacionEnVentana.CenuConsultando Then
            ...
            Else
                With MobjObjetoWin
                    ...
                    txtSaldoMora.Content = Format(.FdecDeudaIntTotal, "c")
    ...
    Protected Overrides Sub SConfigureMenuesPropios()
        ...
        End If
        '
        Dim lstrTitulo As String = "  Mostrar Lista "
        lstrTitulo &= If(GobjParametros.BlnEFacAutorizado, "Facturas", "Ctas Cobro")
        MnuMostrarLista = FmnuiMenuItem("MnuMostrarLista", lstrTitulo, "RecMnuItemPriInf")
...
#Region "Procedimientos invalidantes"
    ...
    Protected Overrides Sub SCree()
        ...
        If ClsOrionCop.FblnPuedeCrear(EnuTipoDocOri.EnuFactura, True, lstrMens) Then
            ...
            SInicialiceObjeto()
            MblnCreando = False
            If cnvConsulta.Visibility = Visibility.Visible Then
                SMuestreFacturas(False)
            End If
            MstrNumeroFac = ""
            MyBase.SCree()
            SPuebleCombos()
    ...
    Protected Overrides Function SAnule() As Boolean
        ...
        If larlNrosFacts.Count > 1 Then
            ...
            If MblnForzarAnulacion Then
                lblnAnulo = SAnuleFactsProps(larlNrosFacts)
            Else
    ...
    Private Function SAnuleFactsProps(aarlFacturasProps As ArrayList) As Boolean
        Dim lstrPref As String, lentIdFact As Integer
        Dim lblnAnulo = False
        Dim lobjValorLlave As Object()
        GobjPanDat.SControleProcesoObj(True)
...
#Region "Procedimientos Propios"
    ...
    Private Sub SMuestreFacturas(ablnMostrar As Boolean)
        If MobjObjetoWin IsNot Nothing Then
            Dim ldtbFacturas = MobjObjetoWin.FdtbFactsUltimoMes
            If ldtbFacturas.Rows.Count = 0 Then
                cnvFacturas.Visibility = Visibility.Hidden
                cnvConsulta.Visibility = Visibility.Visible
                MnuMostrarLista.Visibility = Visibility.Collapsed
            Else
                If ablnMostrar Then
                    cnvFacturas.Visibility = Visibility.Visible
                    cnvConsulta.Visibility = Visibility.Hidden
                    ...
                    SLevanteEveNoti("Doble clic o clic contrario abre el documento seleccionado!",
...
#Region "eFac"
    ...
    Private Sub SHabiliteMenuEFac()
        ...
        If GobjParametros.BlnEFacAutorizado AndAlso MobjObjetoWin.BlnExiste AndAlso
            ...
            If Not HblnCargandoForma AndAlso MobjObjetoWin.BlnEsFacEle Then
    ...
    Private Sub SVerifiqueEstadoEDoc()
        Dim lstrMens As String, lblnAnular As Boolean, lblnRegistrar = False
        ...
        If lenuEstadoEFac < EnuEstadoEDoc.EnuEnviada Then
            If MobjObjetoWin.ObjFechaFacturaDtm.ObjValorPro < Date.Today Then
                lstrMens = "Se debe definir la Factura como no electrónica y anularla!"
                lblnAnular = MsgBox(lstrMens, vbYesNo, "Anular Factura?") = vbYes
            Else
                If lenuEstadoEFac = EnuEstadoEDoc.EnuErrorFtp Then
                    lstrMens = "La Factura no fue aceptada por MisFacturas. " &
                            "Ya esta corregida?"
                    lblnRegistrar = MsgBox(lstrMens, vbYesNo, "Procesar Factura?") = vbYes
                ElseIf lenuEstadoEFac = EnuEstadoEDoc.EnuInvalida Then
                    lstrMens = "La Factura fue rechazada por la DIAN. " &
                            "Ya esta corregida?"
                    lblnRegistrar = MsgBox(lstrMens, vbYesNo, "Procesar Factura?") = vbYes
            ...
            End If
            If lblnAnular Then
                If GobjParametros.ObjAnoActual.ObjPeriodoActual.DtmFechaFinPeriodo <
                        Date.Today Then
                    MsgBox("Es necesario cerrar el mes antes de anular la factura!",
                            vbOKOnly, "Información!")
                Else
                    MobjObjetoWin.SCambieANoFacEle()
                    SAnule()
                End If
            End If
        ElseIf lenuEstadoEFac = EnuEstadoEDoc.EnuRechazada Then
            lstrMens = "Esta Factura fue rechazada por el Cliente." & vbCrLf &
                "Por lo tanto será anulada!"
            MsgBox(lstrMens, vbOKOnly, "Factura rechazada!")
            MblnForzarAnulacion = True
            SAnule()
        ElseIf lenuEstadoEFac <> EnuEstadoEDoc.EnuEnviada Then
            lstrMens = "Por favor informe este estado a soporte en Optimusof!"
...
dtpFechaFactura.LostFocus
    ...
    Private Sub DgrFac_MouseDoubleClick(sender As Object, e As MouseButtonEventArgs) _
            Handles dgrFacturas.MouseDoubleClick
        Dim ldrvFilaActual As DataRowView
        If TypeOf sender Is DataGrid Then
            Dim ldgrActual As DataGrid = sender
            Dim lstrPrefijo As String
            Dim lentIdFact As Integer
            ldrvFilaActual = ldgrActual.SelectedItem
            If Not IsNothing(ldrvFilaActual) AndAlso ldgrActual.SelectedIndex >= 0 Then
                lstrPrefijo = ldrvFilaActual("Prefijo")
                lentIdFact = ldrvFilaActual("IdFactura")
                cboPref.SelectedItem = lstrPrefijo
                txtIdFactura.Text = lentIdFact
                SAbraFact()
            End If
        End If
        SMuestreFacturas(False)
    ...
    Private Sub DgrFac_MouseRightButtonUp(sender As Object, e As MouseButtonEventArgs) _
            Handles dgrFacturas.MouseRightButtonUp
        Dim ldrvFilaActual As DataRowView
        If TypeOf sender Is DataGrid Then
            Dim ldgrActual As DataGrid = sender
            Dim lstrPrefijo As String
