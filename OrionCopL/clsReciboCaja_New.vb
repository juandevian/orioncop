...
#Region "Procedimientos del objeto"
    ...
    Friend Function FdtbRecsCajaUltimoMes() As DataTable
        ...
        Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion_Pri & " AND " & ClsFechaRecDtm.SstrNombreCampoBd &
                " BETWEEN " & lstrFechaIni & " AND " & lstrFechaFin & " AND " &
                ClsAnuladoBln.SstrNombreCampoBd & " = FALSE"
        Dim lstrOrden As String(,) = {{ClsPrefijo_RecStr.SstrNombreCampoBd, "ASC"},
...
#Region "Genera Items Rec Caja"
    ...
    Private Sub SGenereItemsRecPago()
        Dim ldecPagoTotal As Decimal = ObjValor_RecDec.ObjValorPro - ObjValorAnticipoDec.ObjValorPro
        If ldecPagoTotal > 0 Then
            ...
            For Each lobjFactura As ClsFactura In McolFrasVivas
                ...
                If lblnAplicaAFra Then
                    ldecAAplicar = 0
                    ldecMoraFac = lobjFactura.FdecDeudaIntMora
            ...
            If ldecPorAplicar > 0 Then
                ...
                For Each lobjFactura As ClsFactura In McolFrasVivas
                    Dim ldecDeudaCapital = lobjFactura.DecDeudaCapitalSer(lstrServicios)
                    lstrIdPreAgrFac = lobjFactura.ObjIdPredioAgrupador_FacStr.ObjValorPro
    ...
    Private Sub SGenereItemsRecPagoFactCap(aobjFactura As ClsFactura, adecValor As Decimal)
        ...
        For Each lobjItemFac As ClsItemFactura In aobjFactura.ColItemsFactura
            ...
            End If
            If lblnAplicar Then
                ldecAAplicar = If(lobjItemFac.FdecDeudaServicioTotal <= ldecPorAplicar,
                        lobjItemFac.FdecDeudaServicioTotal, ldecPorAplicar)
            End If
...
#Region "Genera Notas Cr, una por Factura"
    ...
    Private Sub SAdicioneItemsNcr(aobjNotaCr As ClsNotaCr, astrNroFac As String)
        ...
        For Each ldrwDscto As DataRow In DtbDescuentos.Rows
            ...
            If lstrNroFac = astrNroFac Then
                ...
                aobjNotaCr.SAdicioneNuevoItem(lobjNewItemNCr)
            End If
        Next
    End Sub
    Private Sub SGenereNCrDsctosMora()
        ...
        Dim lstrFiltro = "IdTipoDscto = " & EnuTipoDescuentoDef.EnuDsctoIntMora.ToString()
...
#Region "Descuentos"
    ...
    Friend EnuTipoDscto As EnuTipoDescuentoDef
    ...
    Private Function FblnHayDscto(astrNroFact As String, ashrIdItemFac As Short,
             aenuTipoDscto As EnuTipoDescuentoDef) As Boolean
        Dim lstrNroFac As String, lenuTipoDscto As EnuTipoDescuentoDef
        Dim lstrItemFac As String, lshrIdItemFac As Short
    ...
    Friend Function FblnHayDsctoPP() As Boolean
        Dim lblnHay = False
        If Not IsNothing(MdtbDescuentos) Then
            Dim lenuTipoDsct As EnuTipoDescuentoDef
            For Each ldrwDscto In MdtbDescuentos.Rows
                ...
                If lenuTipoDsct = EnuTipoDescuentoDef.EnuDsctoPP Then
                    lblnHay = True
                    Exit For
                End If
            Next
    ...
    Private Function FblnEsValidoDscto(astrNroFactura As String, ashrIdItemFac As Short,
             aenuTipoDescuento As EnuTipoDescuentoDef, adecValorDscto As Decimal,
        ...
        End If
        If lblnEsValido Then
            lblnEsValido = (aenuTipoDescuento <> EnuTipoDescuentoDef.EnuCancelaIva)
            If Not lblnEsValido Then
