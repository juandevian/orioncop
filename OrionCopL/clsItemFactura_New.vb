...
#Region "Valores del servicio"
    ' Valor del Servicio (Facturado - IVA)
    ''' <summary>
    ''' Devuelve el valor del servicio sin tener en cuenta el IVA
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecValorServicio() As Decimal
        Dim ldecValorServicio As Decimal
        ...
        If ldblTasaIva > 0 Then
            ldecValorServicio = Math.Round(ldecValor / (1 + ldblTasaIva))
        Else
            ldecValorServicio = ldecValor
        End If
        Return ldecValorServicio
    End Function
    ' Valor del IVA al Servicio
    ''' <summary>
    ''' Devuelve la valor del iva incluido en el valor del item
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecIvaServicio() As Decimal
        ...
        Return ldecIva
    End Function
    ' Base del IVA Servicio
    ''' <summary>
    ''' Devuelve la base del IVA. Si la tasa de iva es cero, la base será cero
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecBaseIvaServicio() As Decimal
        Dim ldecBaseIva = 0D
        Dim ldecValor As Decimal = ObjValor_ItemFactDec.ObjValorPro
        Dim ldblTasaIva As Double = ObjTarifaIva_ItemFactDbl.ObjValorPro
        If ldblTasaIva > 0 Then
            ldecBaseIva = Math.Round(ldecValor / (1 + ldblTasaIva))
        End If
        Return ldecBaseIva
    End Function
    ' Valor de la Mora (Generdo - IVA)
    ''' <summary>
    ''' Devuelve el total de intereses causados. Se usa para la nota crédito por anulación de factura
    ''' ya que solo es posible anular una factura cuando no tiene créditos.
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecIntMora() As Decimal
        Dim ldecIntMora = 0D
        For Each lobjNov As ClsNovedad In ColNovedades
            If lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuDbInt Then
                ldecIntMora += lobjNov.ObjValor_NovDec.ObjValorPro
            End If
        Next
        Return ldecIntMora
    End Function
    ' Valor del IVA a la Mora
    Friend Function FdecIvaInt() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            .Append("SELECT SUM(IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuDbIvaInt).Append(", Valor, 0) - IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRDbIvaInt).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCarpeta).Append(" AND ").Append(ClsIdCentroUtilShr.SstrNombreCampoBd)
            .Append(" = ").Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd).Append(" = '")
            .Append(lstrPref).Append("' AND ").Append(ClsIdFactura_NovEnt.SstrNombreCampoBd)
            .Append(" = ").Append(lentIdFac).Append(" AND ")
            .Append(ClsIdItemFacturaShr.SstrNombreCampoBd).Append(" = ").Append(lshrIdItem)
        End With
        Dim lstrSql = lstbSql.ToString
        Dim ldtbRes = ClsPanorama.FdtbDataTable(lstrSql)
        Dim ldecIvaGeneradoInt = ClsPanorama.FobjValorCampo(ldtbRes.Rows(0)("Saldo"),
                EnuTipoValor.EnuDecimal)
        Return ldecIvaGeneradoInt
    End Function
    ' Base del IVA a la Mora
    ''' <summary>
    ''' Devuleve la deuda total, Iva incluido, de los intereses gravados con Iva
    ''' </summary>
    ''' <returns></returns>
    ''' <summary>
    ''' Devuelve la bse del iva  para aquellos intereses que causaron Iva. 
...
#Region "Deuda"
    ...
    End Property
    ''' <summary>
    ''' Devuelve la deuda del servicio incluido el IVA tanto al servicio como a los intereses
    ''' </summary>
    ''' <returns></returns>
    Private Function FdecDeudaServicioTot() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            .Append("SELECT SUM(IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuDbCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuDbIva).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuDbIvaInt).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrPagoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrAnApCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrDctoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrIvaGas).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" BETWEEN ")
            .Append(EnuTipoNov.EnuRCrRetFte).Append(" AND ").Append(EnuTipoNov.EnuRCrRetCre)
            .Append(", Valor, 0) - IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrPagoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrAnApCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrDctoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" BETWEEN ")
            .Append(EnuTipoNov.EnuCrRetFte).Append(" AND ").Append(EnuTipoNov.EnuCrRetCre).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRDbCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRDbIva).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRDbIvaInt).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrIvaGas).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCarpeta).Append(" AND ")
            .Append(ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd).Append(" = '")
            .Append(lstrPref).Append("' AND ")
            .Append(ClsIdFactura_NovEnt.SstrNombreCampoBd).Append(" = ")
            .Append(lentIdFac).Append(" AND ")
            .Append(ClsIdItemFacturaShr.SstrNombreCampoBd).Append(" = ").Append(lshrIdItem)
        End With
        Dim lstrSql = lstbSql.ToString
        Dim ldtbDeuda = ClsPanorama.FdtbDataTable(lstrSql)
        Dim ldecDeuda = ClsPanorama.FobjValorCampo(ldtbDeuda.Rows(0)("Saldo"), EnuTipoValor.EnuDecimal)
        Return ldecDeuda
    End Function
    ''' <summary>
    ''' Devuelve el valor de la deuda del servicio incluido el IVA al momento de la última 
    ''' lectura de las novedades
    ''' </summary>
    Friend Function FdecDeudaServicioTotal() As Decimal
        Dim ldecDeudaCapital = FdecDeudaServicioTot() - MdecCrAplicadoCap
        Return ldecDeudaCapital
    End Function

    ''' <summary>
    ''' Devuelve el valor de la deuda por mora incluido el IVA al momento de la última 
    ''' lectura de las novedades
    ''' </summary>
    ' Deuda intereses
    Friend Function FdecDeudaIntMora() As Decimal
        Dim ldecDeudaMora = FdecDeudaIntTotal() - MdecCrAplicadoInt
    ...
    ''' </summary>
    ''' <returns></returns>
    Private Function FdecDeudaIntTotal() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref As String = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac As Integer = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            .Append("SELECT SUM(IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            ...
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRDbInt)
            .Append(", Valor, 0)) AS Saldo FROM ").Append(ClsNovedad.SstrNombreTabla)
            .Append(" WHERE ").Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCarpeta).Append(" AND ").Append(ClsIdCentroUtilShr.SstrNombreCampoBd)
            .Append(" = ").Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd).Append(" = '")
            .Append(lstrPref).Append("' AND ").Append(ClsIdFactura_NovEnt.SstrNombreCampoBd)
        ...
        Return ldecDeuda
    End Function
    ' Deuda total de la Mora
    ' + EnuRCrDctoInt
    'EnuCrPagoInt + EnuCrAnApInt + EnuCrDctoInt + EnuRDbInt + EnuRDbIvaInt




    ' AVV Revisar lo relacionado con la deuda del IVA
    ' Deuda Iva
    ''' <summary>
    ''' Devuelve la deuda del IVA donde los pagos se aplican primero al IVA
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecDeudaIva() As Decimal
        Dim ldecIVA = FdecIvaServicio() + FdecIvaInt()
        Dim ldecPagos = FDecPagos()
        Dim ldecDeudaIva As Decimal = If(ldecPagos >= ldecIVA, 0, ldecIVA - ldecPagos)
        Return ldecDeudaIva
    End Function
    ...
    ''' <summary>
    ''' Devuelve el total debido de IVA a los intereses
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecDeudaIvaInt() As Decimal
        Dim ldecDeudaIvaInt = 0D, ldecPagoInt = 0D, lenuTipoNov As EnuTipoNov
            ...
            For Each lobjNov As ClsNovedad In ColNovedades
                ...
                Select Case lenuTipoNov
                    Case EnuTipoNov.EnuDbIvaInt
                        ldecDeudaIvaInt += lobjNov.ObjValor_NovDec.ObjValorPro
                    Case EnuTipoNov.EnuCrAnApInt, EnuTipoNov.EnuCrPagoInt
                        ldecPagoInt += lobjNov.ObjValor_NovDec.ObjValorPro
                    Case EnuTipoNov.EnuRCrAnApInt, EnuTipoNov.EnuRCrPagoInt
                        ldecPagoInt -= lobjNov.ObjValor_NovDec.ObjValorPro
                End Select
            Next
            If ldecPagoInt >= ldecDeudaIvaInt Then
                ldecDeudaIvaInt = 0
            Else
                ldecDeudaIvaInt -= ldecPagoInt
            End If
        End If
        Return ldecDeudaIvaInt
    End Function


    Private Function FdecIva() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            ...
            .Append(EnuTipoNov.EnuRDbIvaInt).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCarpeta).Append(" AND ")
            .Append(ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd).Append(" = '").Append(lstrPref).Append("' AND ")
            .Append(ClsIdFactura_NovEnt.SstrNombreCampoBd).Append(" = ").Append(lentIdFac).Append(" AND ")
            .Append(ClsIdItemFacturaShr.SstrNombreCampoBd).Append(" = ").Append(lshrIdItem)
        End With
        Dim lstrSql = lstbSql.ToString
        Dim ldtbRes = ClsPanorama.FdtbDataTable(lstrSql)
        Dim ldecIva = ClsPanorama.FobjValorCampo(ldtbRes.Rows(0)("Saldo"), EnuTipoValor.EnuDecimal)
        Return ldecIva

    End Function
    Private Function FdecPagos() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            ...
            .Append(EnuTipoNov.EnuCrPagoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrAnApCap).Append(", Valor, 0) - IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrPagoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrAnApCap).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCarpeta).Append(" AND ")
            .Append(ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd).Append(" = '").Append(lstrPref).Append("' AND ")
        ...
        Dim lstrSql = lstbSql.ToString
        Dim ldtbDeuda = ClsPanorama.FdtbDataTable(lstrSql)
        Dim ldecPagos = ClsPanorama.FobjValorCampo(ldtbDeuda.Rows(0)("Saldo"), EnuTipoValor.EnuDecimal)
        Return ldecPagos
    End Function
    Private Function FdecDescuentos() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            ...
            .Append(EnuTipoNov.EnuCrDctoInt).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrRetFte).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrRetIca).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrRetCre).Append(", Valor, 0) - IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrDctoCap).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrDctoInt).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrRetFte).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrRetIva).Append(" OR ")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrRetCre).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCarpeta).Append(" AND ")
            .Append(ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsPrefijoFact_NovStr.SstrNombreCampoBd).Append(" = '").Append(lstrPref).Append("' AND ")
        ...
        Dim ldecDsctos = ClsPanorama.FobjValorCampo(ldtbResul.Rows(0)("Saldo"), EnuTipoValor.EnuDecimal)
        Return ldecDsctos
    End Function
#End Region
#Region "Procedimientos del objeto"
    ...
    End Sub
    Friend Function FblnRetencionAplicada(aenuTipoDscto As EnuTipoDescuentoDef) As Boolean
    ...
    Friend Function FdecPagoTotalAplicado() As Decimal
        ...
        Return ldecVlrPago
    End Function
    Friend Function FdblTasaIva() As Double
        Dim ldblTasaIva = 0.0
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
            ldblTasaIva = ObjServicio.ObjTarifaIvaDbl.ObjValorPro
        Else
            For Each lobjNov As ClsNovedad In ColNovedades
                If lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuDbIva OrElse
                        lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuRDbIva OrElse
                        lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuCrIvaGas Then
                    ldblTasaIva = lobjNov.ObjFactorDbl.ObjValorPro
                    If ldblTasaIva > 0 Then Exit For
                End If
            Next
        End If
        Return ldblTasaIva
    ...
    End Function
    Friend Function FdblTasaReteIva() As Double
        Dim ldblTasaRetIva = 0.0
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
            ldblTasaRetIva = GobjParametros.ObjTarifaReteIvaDbl.ObjValorPro
        Else
            For Each lobjNov As ClsNovedad In ColNovedades
                If lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuCrRetIva OrElse
                        lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuRCrRetIva Then
                    ldblTasaRetIva = lobjNov.ObjFactorDbl.ObjValorPro
                    If ldblTasaRetIva > 0 Then Exit For
                End If
            Next
        End If
        Return ldblTasaRetIva
    End Function
    Friend Function FdblTasaReteFte() As Double
        Dim ldblTasaRetFte = 0.0
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
            ldblTasaRetFte = ObjServicio.ObjTarifaRetFteDbl.ObjValorPro
        Else
            For Each lobjNov As ClsNovedad In ColNovedades
                If lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuCrRetFte OrElse
                        lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuRCrRetFte Then
                    ldblTasaRetFte = lobjNov.ObjFactorDbl.ObjValorPro
                    If ldblTasaRetFte > 0 Then Exit For
                End If
            Next
        End If
        Return ldblTasaRetFte
    End Function
    Friend Function FdblTasaReteIca() As Double
        Dim ldblTasaRetIca = 0.0
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
            ldblTasaRetIca = ObjServicio.ObjTarifaRetIcaDbl.ObjValorPro
        Else
            For Each lobjNov As ClsNovedad In ColNovedades
                If lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuCrRetIca OrElse
                        lobjNov.ObjIdTipoNovedadByt.ObjValorPro = EnuTipoNov.EnuRCrRetIca Then
                    ldblTasaRetIca = lobjNov.ObjFactorDbl.ObjValorPro
                    If ldblTasaRetIca > 0 Then Exit For
                End If
            Next
        End If
        Return ldblTasaRetIca
    End Function
#End Region
...
#Region "Manejo Novedades"
    ...
    Private Sub SGenereNovedad_CxC_Ing()
        ...
        With lobjNovedad
            ...
            .ObjValor_NovDec.ObjValorPro = FdecValorServicio()
    ...
    Private Sub SGenereNovedad_CxC_Iva()
        ...
        Dim lobjNovedad As ClsNovedad = FobjNuevaNovedad()
        With lobjNovedad
            .ObjBaseDec.ObjValorPro = FdecBaseIvaServicio()
            ...
            .ObjValor_NovDec.ObjValorPro = FdecIvaServicio()
        End With
        McolNovedades.Add(lobjNovedad)
    End Sub
    ''' <summary>
...
#Region "Calculo intereses mora"
    ...
    Private Function FdecIntereseMora(adtmFechaCalculo As Date,
            ByRef aentDiasMora As Integer) As Decimal
        Dim ldecVlrMora = 0D
        ...
        If aentDiasMora > 0 Then
            ...
            ldecVlrMora = FdecDeudaServicioTotal() * (ldblTasaMora / lentDiasAno) * aentDiasMora
...
#Region "Retenciones y Descuento por Pronto Pago"
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Friend Function FdecValorRetencion(aenuTipoRetencion As EnuTipoDescuentoDef,
    ...
    Friend Function FdecValorRetencion(aenuTipoRetencion As EnuTipoDescuentoDef)
        ...
        Select Case aenuTipoRetencion
            Case EnuTipoDescuentoDef.EnuReteFuente
                ldecVlrRetencion = FdecReteFuente(ldecBaseRet, ldblTasaRet)
            Case EnuTipoDescuentoDef.EnuReteIca
                ldecVlrRetencion = FdecReteIca(ldecBaseRet, ldblTasaRet)
            Case EnuTipoDescuentoDef.EnuReteCree
                ldecVlrRetencion = 0
            Case EnuTipoDescuentoDef.EnuReteIva
                ldecVlrRetencion = FdecReteIva(ldecBaseRet, ldblTasaRet)
        End Select
        Return ldecVlrRetencion
    End Function
    ''' <summary>
    ...
    Friend Function FdecDsctoPPPosible() As Decimal
        ...
        If GobjParametros.ObjAnoActual.ObjAplicaDsctoPPBln.ObjValorPro AndAlso
            ...
            lobjPredio.SAbra(lobjValorLlave)
            If ObjServicio.ObjEsAjusteBln.ObjValorPro Then
                ldecDsctoPP = ClsOrionCop.FdecAjusteDsctoPP(lobjPredio,
    ...
    Private Function FdecReteFuente(ByRef adecBaseRet As Decimal, ByRef adblTasaRet As Double) As Decimal
        ...
        If MobjMiFactura.ObjClienteFactura.ObjEsAgenteReteFteBln.ObjValorPro Then
            ...
            If adblTasaRet > 0 Then
                If Not FblnRetencionAplicada(EnuTipoDescuentoDef.EnuReteFuente) Then
                    Dim ldecBaseMinRet As Decimal = ObjServicio.ObjBaseMinimaReteFuenteDec.ObjValorPro
                    adecBaseRet = FdecValorServicio()
                    If adecBaseRet >= ldecBaseMinRet Then
    ...
    Private Function FdecReteIca(ByRef adecBaseRet As Decimal, ByRef adblTasaRet As Double) As Decimal
        ...
        If MobjMiFactura.ObjClienteFactura.ObjRetieneIcaBln.ObjValorPro Then
            If Not FblnRetencionAplicada(EnuTipoDescuentoDef.EnuReteIca) Then
    ...
    Private Function FdecReteIva(ByRef adecBaseRet As Decimal, ByRef adblTasaRet As Double) As Decimal
        ...
        If MobjMiFactura.ObjClienteFactura.ObjRetieneIvaBln.ObjValorPro Then
            If Not FblnRetencionAplicada(EnuTipoDescuentoDef.EnuReteIva) Then
                adecBaseRet = FdecIvaServicio()
                If adecBaseRet > 0 Then
...
#Region "Datos EFactura"
    ...
    Friend ReadOnly Property StrPrice As String
        Get
            Return Format(FdecValorServicio, "#0.00")
        End Get
    End Property
    ''' <summary>
    ...
    Friend ReadOnly Property StrLineExtensionAmount As String
        Get
            Dim ldecRes As Decimal = FdecValorServicio()
    ...
    Friend ReadOnly Property StrLineTotalTaxes
        Get
            Return Format(FdecIvaServicio, "#0.00")
        End Get
    End Property
    ''' <summary>
