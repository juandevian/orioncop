...
#Region "Prefacturar"
    ...
    Private Shared Sub SAddItemFact(aobjFactura As ClsFactura, astrIdPredio As String,
        ...
        Dim lblnHayConsumo As Boolean = FblnHayConsumoOk(astrIdPredio, lshrIdServ)
        Dim lstrDetalle = lobjServicio.ObjNombreServicioStr.ObjValorPro & " " & astrIdPredio
        If lblnHayConsumo AndAlso lshrIdAno = 0 Then
            Dim ldecLecturaAnterior As Decimal = 0
            Dim ldecLecturaActual As Decimal = 0
            Dim ldecValorUnitario As Decimal = 0
            Dim ldecConsumo As Decimal = 0
            Dim lstrCamposSelect As String() = {
                    ClsLecturaActual_ItemProgramaFactDec.SstrNombreCampoBd,
                    ClsLecturaAnterior_ItemProgramaFactDec.SstrNombreCampoBd,
                    ClsValorUnitario_ItemProgramaFactDec.SstrNombreCampoBd}
            Dim lstrOrden As String(,) = {{String.Empty, String.Empty}}
            Dim lstrFiltro As String = ClsIdCarpetaShr.SstrNombreCampoBd & " = " &
                    GshrIdCarpeta & " AND " & ClsIdCentroUtilShr.SstrNombreCampoBd & " = " &
                    GshrIdCentroUtil & " AND " &
                    ClsIdPredio_ItemProgramaFactStr.SstrNombreCampoBd & " = '" &
                    astrIdPredio & "' AND " & ClsIdAno_ItemProgramaFactShr.SstrNombreCampoBd &
                    " = " & lshrIdAno & " AND " &
                    ClsIdServicio_ItemProgramaFactShr.SstrNombreCampoBd & " = " & lshrIdServ
            Dim ldtbServicioConsumo As DataTable =
                    ClsPanorama.FdtbDataTable(ClsItemProgramaFact.SstrNombreTabla,
                    lstrCamposSelect, lstrOrden, lstrFiltro)
            ldecLecturaAnterior = ClsPanorama.FobjValorCampo(ldtbServicioConsumo.Rows(0)(
                    ClsLecturaAnterior_ItemProgramaFactDec.SstrNombreCampoBd),
                    EnuTipoValor.EnuDecimal)
            ldecLecturaActual = ClsPanorama.FobjValorCampo(ldtbServicioConsumo.Rows(0)(
                    ClsLecturaActual_ItemProgramaFactDec.SstrNombreCampoBd),
                    EnuTipoValor.EnuDecimal)
            ldecValorUnitario = ClsPanorama.FobjValorCampo(ldtbServicioConsumo.Rows(0)(
                    ClsValorUnitario_ItemProgramaFactDec.SstrNombreCampoBd),
                    EnuTipoValor.EnuDecimal)
            ldecConsumo = ldecLecturaActual - ldecLecturaAnterior
            lstrDetalle &= " Lectura Ant/Act: " & ldecLecturaAnterior.ToString() & "/" &
                    ldecLecturaActual.ToString() &
                    " (Consumo: " & CInt(ldecConsumo).ToString("N0") & " x " &
                    CInt(ldecValorUnitario).ToString("C0") & ")"
        End If
    ...
    Private Shared Function FblnHayConsumoOk(astrIdPredio As String, ashrIdServ As Short) As Boolean
        ...
        Dim lstrOrden As String(,) = {{String.Empty, String.Empty}}
        Dim lstrFiltro As String = ClsIdCarpetaShr.SstrNombreCampoBd & " = " & GshrIdCarpeta &
                " AND " & ClsIdCentroUtilShr.SstrNombreCampoBd & " = " & GshrIdCentroUtil &
                " AND " & ClsIdPredio_ItemProgramaFactStr.SstrNombreCampoBd & " = '" &
                astrIdPredio & "' AND " & ClsIdServicio_ItemProgramaFactShr.SstrNombreCampoBd &
                " = " & ashrIdServ
        Dim ldtbServicioConsumo As DataTable = ClsPanorama.FdtbDataTable(
                ClsItemProgramaFact.SstrNombreTabla, lstrCamposSelect, lstrOrden, lstrFiltro)
        If ldtbServicioConsumo.Rows.Count = 0 Or ClsPanorama.FobjValorCampo(
                ldtbServicioConsumo.Rows(0)(ClsConsumo_ItemProgramaFactDec.SstrNombreCampoBd),
                EnuTipoValor.EnuDecimal) <= 0 Then
            Return False
        Else
            Return True
        End If
...
#Region "Generar Facturas Definitivas"
    ...
    Friend Shared Function FblnClientesSinEmail(ByRef astrMens As String) As Boolean
        Dim ldtbClientesSinEmail = FdtbClientesSinEmail()
        Dim lblnHayClientesSinEmail = ldtbClientesSinEmail.Rows.Count > 0
        If lblnHayClientesSinEmail Then
            astrMens = "Los siguientes clientes no tienen Email registrado." & vbCrLf
            For Each ldrwCliente As DataRow In ldtbClientesSinEmail.Rows
                astrMens &= "Id Cliente: " & ClsPanorama.FobjValorCampo(ldrwCliente(
                        "IdCliente"), EnuTipoValor.EnuString) & " - Nombre: " &
                        ClsPanorama.FobjValorCampo(ldrwCliente(
                        ClsNombreCompletoStr.SstrNombreCampoBd), EnuTipoValor.EnuString) &
                        vbCrLf
            Next
        End If
        Return lblnHayClientesSinEmail
    End Function

    Private Shared Function FdtbClientesSinEmail() As DataTable
        Dim lstbSql As New StringBuilder
        With lstbSql
            .Append("(SELECT DISTINCT(").Append(ClsIdCliente_PropDbl.SstrNombreCampoBd)
            .Append(") AS IdCliente, ").Append(ClsNombreCompleto_PropStr.SstrNombreCampoBd)
            .Append(" FROM ").Append(ClsPropietario.SstrNombreTabla).Append(" AS P INNER JOIN ")
            .Append(ClsTercero.SstrNombreTabla).Append(" AS T ON P.")
            .Append(ClsIdCliente_PropDbl.SstrNombreCampoBd).Append(" = T.")
            .Append(ClsIdTerceroDbl.SstrNombreCampoBd).Append(" WHERE P.")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCarpeta)
            .Append(" AND P.").Append(ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCentroUtil).Append(" AND ").Append(ClsEmailStr.SstrNombreCampoBd)
            .Append(" IS NULL) UNION ALL (SELECT G.IdCliente, C.")
            .Append(ClsNombreCompletoStr.SstrNombreCampoBd).Append(" FROM (SELECT DISTINCT(I.")
            .Append(ClsIdCliente_ItemProgramaFactDbl.SstrNombreCampoBd)
            .Append(") AS IdCliente FROM ").Append(ClsItemProgramaFact.SstrNombreTabla)
            .Append(" AS I INNER JOIN ").Append(ClsTercero.SstrNombreTabla).Append(" AS T ON I.")
            .Append(ClsIdCliente_ItemProgramaFactDbl.SstrNombreCampoBd).Append(" = T.")
            .Append(ClsIdTerceroDbl.SstrNombreCampoBd).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ").Append(GshrIdCarpeta)
            .Append(" AND ").Append(ClsIdCentroUtilShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCentroUtil).Append(" AND ")
            .Append(ClsSaldo_ItemProgramaFactDec.SstrNombreCampoBd).Append(" > 0 AND ")
            .Append(ClsEmailStr.SstrNombreCampoBd).Append(" IS NULL) AS G ")
            .Append(" INNER JOIN ").Append(ClsCliente.SstrNombreTabla)
            .Append(" AS C ON G.IdCliente = C.")
            .Append(ClsIdClienteDbl.SstrNombreCampoBd).Append(") ORDER BY IdCliente")
        End With
        Dim ldtbRes = ClsPanorama.FdtbDataTable(lstbSql.ToString)
        Return ldtbRes
    End Function
    ...
    Private Shared Sub SApliqueAnticipoFac(aobjFactura As ClsFactura, astrServicios As String(),
            ...
            astrIdpreAgr As String, aobjAnticipo As ClsAnticipo,
            ByRef adecVlrAplicado As Decimal)
        ...
        Dim lstrPeriodoFact = ClsPanorama.FstrPeriodo(
                aobjFactura.ObjFechaFacturaDtm.ObjValorPro)
        Dim lblnAplicar As Boolean = aobjFactura.ObjPrefijo_FactStr.ObjValorPro <>
                GCSTRPREFPREFACTURA
        If lblnAplicar Then
    ...
    Private Shared Sub SGenereItemsNotaCon(aobjNotaCon As ClsNotaCon, aobjFactura As ClsFactura,
            adecValorPorAplicar As Decimal, ashrIdItemFcat As Short, ByRef adecValorAplicado As Decimal)
        ...
        For Each lobjItemFac As ClsItemFactura In aobjFactura.ColItemsFactura
            If ashrIdItemFcat = 0 OrElse lobjItemFac.ObjIdItemFacturaShr.ObjValorPro = ashrIdItemFcat Then
                ldecVlrAAplicar = 0
                If Not lobjItemFac.FblnRetencionAplicada(EnuTipoDescuento.EnuDsctoPP) Then
                    ldecDsctoPP = lobjItemFac.FdecDsctoPPAAplicar(aobjFactura.ObjFechaFacturaDtm.ObjValorPro)
                End If
                ldecDeudaCapItemFac = lobjItemFac.FdecDeudaCapital
                If ldecDsctoPP > 0 AndAlso ldecDeudaCapItemFac > 0 Then
                    ...
                    End If
                End If
                Dim ldecDeudaMora As Decimal = lobjItemFac.FdecDeudaIntTotal
    ...
    ' Genera los Items de la Nota Contable para la aplicación de un aticipo generado
    ' en ajuste Cuotas admin
    Private Shared Sub SGenereItemsNotaCon(aobjNotaCon As ClsNotaCon, aobjFactura As ClsFactura,
...
#Region "Procedimientos eFac"
    ...
    Friend Shared Function FblnHayFacsFueraFecha() As Boolean
        Dim lblnHayFactFFecha = GobjParametros.BlnEFacAutorizado
        If Not lblnHayFactFFecha Then
            Return False
        End If
        Dim lstrTabla = ClsFactura.SstrNombreTabla
        Dim lstrCampSele = {"COUNT(*) AS Tot"}
        Dim lstrFechaHoy = ClsPanoramaDat.FstrFechaNormalizada(Today)
        Dim lstrFiltro = StrFiltroUbicacion & " AND " &
                ClsIdEstadoEDocEnt.SstrNombreCampoBd &
                " < " & EnuEstadoEDoc.EnuEnProceso & " AND " &
                ClsFechaFacturaDtm.SstrNombreCampoBd & " <> '" & lstrFechaHoy & "'"
        Dim ldtbResu = ClsPanorama.FdtbDataTable(lstrTabla, lstrCampSele, {{"", ""}}, lstrFiltro)
        Return ldtbResu.Rows(0)(0) > 0
    End Function
    ...
    Friend Shared Function FdtbDocsEstadoCero() As DataTable
        ...
        Dim ldtmFechaDesde As Date = Today.AddDays(-30)
        Dim lstrFechaDesde = "'" & ClsPanoramaDat.FstrFechaNormalizada(ldtmFechaDesde.ToString) & "'"
        Dim lstrFechaHoy = "'" & ClsPanoramaDat.FstrFechaNormalizada(Today.ToString) & "'"
    ...
    Friend Shared Function FstrDocsRechazados() As String
        Dim lstrDocsRechazados As String = String.Empty
        Dim ldtbDocsRechazados = FdtbDocsEFac(EnuTipoDocOri.EnuFactura,
                EnuEstadoEDoc.EnuRechazada)
        For Each ldrwIdDoc As DataRow In ldtbDocsRechazados.Rows
            Dim lstrPref = ClsPanorama.FobjValorCampo(ldrwIdDoc(
                    ClsPrefijo_FactStr.SstrNombreCampoBd), EnuTipoValor.EnuString)
            Dim lentIdFact = ClsPanorama.FobjValorCampo(ldrwIdDoc(
                    ClsIdFacturaEnt.SstrNombreCampoBd), EnuTipoValor.EnuInteger)
            lstrDocsRechazados &= "Fact: " & lstrPref & "-" & lentIdFact & "; "
        Next
        ldtbDocsRechazados = FdtbDocsEFac(EnuTipoDocOri.EnuNotaCr,
                EnuEstadoEDoc.EnuRechazada)
        For Each ldrwIdDoc As DataRow In ldtbDocsRechazados.Rows
            Dim lstrPref = ClsPanorama.FobjValorCampo(ldrwIdDoc(
                    ClsPrefijo_NotaCrStr.SstrNombreCampoBd), EnuTipoValor.EnuString)
            Dim lentIdNota = ClsPanorama.FobjValorCampo(ldrwIdDoc(
                    ClsIdNotaCrEnt.SstrNombreCampoBd), EnuTipoValor.EnuInteger)
            If String.IsNullOrEmpty(lstrPref) Then
                lstrDocsRechazados &= "NotaCr: " & lentIdNota & "; "
            Else
                lstrDocsRechazados &= "NotaCr: " & lstrPref & "-" & lentIdNota & "; "
            End If
        Next
        ldtbDocsRechazados = FdtbDocsEFac(EnuTipoDocOri.EnuNotaDb,
                EnuEstadoEDoc.EnuRechazada)
        For Each ldrwIdDoc As DataRow In ldtbDocsRechazados.Rows
            Dim lstrPref = ClsPanorama.FobjValorCampo(ldrwIdDoc(
            ...
            If String.IsNullOrEmpty(lstrPref) Then
                lstrDocsRechazados &= "NotaDb: " & lentIdNota & "; "
            Else
                lstrDocsRechazados &= "NotaDb: " & lstrPref & "-" & lentIdNota & "; "
            End If
        Next
        Return lstrDocsRechazados
    End Function

    Friend Shared Sub SProcesarFacsFueraFecha(ByRef astrMens As String)
        If FblnHacerCierreMes() Then
            astrMens = "Es necesario hacer el cierre de mes antes de procesar las Facturas fuera de Fecha!."
            Exit Sub
        End If
        Dim ldtbFactFueraFecha = FdtbFacturasFuerafecha()
        Dim lstrPref As String, lentIdFact As Integer
        Dim lobjFac As New ClsFactura()
        For Each ldrwIdFac As DataRow In ldtbFactFueraFecha.Rows
            lstrPref = ClsPanorama.FobjValorCampo(ldrwIdFac(ClsPrefijo_FactStr.SstrNombreCampoBd),
                    EnuTipoValor.EnuString)
            lentIdFact = ClsPanorama.FobjValorCampo(ldrwIdFac(ClsIdFacturaEnt.SstrNombreCampoBd),
                    EnuTipoValor.EnuInteger)
            lobjFac.SAbra({GshrIdCarpeta, GshrIdCentroUtil, lstrPref, lentIdFact})
            lobjFac.EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuModificando
            lobjFac.ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuNoEDoc
            lobjFac.SActualice(True)
            lobjFac.SHabiliteAnular()
            lobjFac.SAnule()
        Next
    End Sub

    Private Shared Function FdtbFacturasFuerafecha() As DataTable
        Dim lstrTabla = ClsFactura.SstrNombreTabla
        Dim lstrCampSele = {ClsPrefijo_FactStr.SstrNombreCampoBd,
                ClsIdFacturaEnt.SstrNombreCampoBd}
        Dim lstrFechaHoy = ClsPanoramaDat.FstrFechaNormalizada(Today)
        Dim lstrFiltro = StrFiltroUbicacion & " AND " & ClsIdEstadoEDocEnt.SstrNombreCampoBd &
                " < " & EnuEstadoEDoc.EnuEnProceso & " AND " &
            ...
            If ldrwItemFacViva("Proc") = "F" Then
                ...
                If lshrIdAno = 0 Then
                    lshrIdServ = ClsPanorama.FobjValorCampo(ldrwItemFacViva(
                            ClsIdServicio_ItemFactShr.SstrNombreCampoBd),
                            EnuTipoValor.EnuShort)
                    If aobjFact.FblnFacturoServicio(lshrIdAno, lshrIdServ, False) Then
...
#Region "Estado de Cuenta de Predios"
    ...
    Private Sub SGenereEstCtaPreAgrPorSer(aobjPreFactura As ClsFactura, adtbFactVivas As DataTable,
        ...
        Dim lstrPrefFacViva As String, lentIdFacViva As Integer
        Dim lshrIdAno As Short, lshrIdServ As Short, lblnEsSerAjuste = False
        ...
        For Each ldrwItemFacViva As DataRow In adtbFactVivas.Rows
            If ldrwItemFacViva("Proc") = "F" Then
                ...
                If lshrIdAno > 0 Then
                    lblnEsSerAjuste = FblnEsServicioAjuste(lshrIdAno, lshrIdServ)
                End If
                If aobjPreFactura.FblnFacturoServicio(lshrIdAno, lshrIdServ, lblnEsSerAjuste) Then
                    lstrPrefFacViva = ClsPanorama.FobjValorCampo(ldrwItemFacViva(
                            ClsPrefijo_ItemFactStr.SstrNombreCampoBd),
                            EnuTipoValor.EnuString)
                    lentIdFacViva = ClsPanorama.FobjValorCampo(ldrwItemFacViva(
            ...
            If ldrwItemFacViva("Proc") = "F" Then
                ...
                If lshrIdAno = 0 Then
                    lshrIdServ = ClsPanorama.FobjValorCampo(ldrwItemFacViva(
                            ClsIdServicio_ItemFactShr.SstrNombreCampoBd),
                            EnuTipoValor.EnuShort)
                    If aobjPrefac.FblnFacturoServicio(lshrIdAno, lshrIdServ, False) Then
                        lstrPrefFacViva = ClsPanorama.FobjValorCampo(ldrwItemFacViva(
                                ClsPrefijo_ItemFactStr.SstrNombreCampoBd),
                                EnuTipoValor.EnuString)
                        lentIdFacViva = ClsPanorama.FobjValorCampo(ldrwItemFacViva(
        ...
        For Each ldrwDscto As DataRow In ldtbDsctosPredio.Rows
            ...
            If lenuTipoDocOri = EnuTipoDocOri.EnuNotaCr Then
                ...
                If lobjItemNotaCr.ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro =
                        EnuTipoDescuento.EnuDsctoCapital Then
    ...
    Private Shared Function FdecDsctoRev(aobjNotaCr As ClsNotaCr) As Decimal
        ...
        For Each lobjItemNCr As ClsItemNotaCr In aobjNotaCr.ColItemsNotaCr
            If lobjItemNCr.ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro =
                    EnuTipoDescuento.EnuDsctoPP Then
...
#Region "Varios"
    ...
    ''' <summary>
    ''' Devuelve la tasa efectiva anual para un valor en porcentaje seguida de dos caracteres 
    ''' que indican el primero la periodicidad y el segundo el modo de pago 
    ''' (Vnecido o anticipado)
    ''' </summary>
    ...
    Friend Shared Function FstrIdCtaDbDscto(aenuTipoDscto As EnuTipoDescuento) As String
...
' Anticipos
    ...
    Friend Shared Function FblnPuedeCrear(aenuTipoDoc As EnuTipoDocOri,
            ablnFactManual As Boolean, ByRef astrMens As String) As Boolean
        ...
        If GobjParametros.BlnEFacAutorizado Then
            ...
            If Not lblnPuede Then
                astrMens = "En EFac se requiere que la Opción: " & Chr(34) &
                        "Documentos deben ser expedidos con Fecha de Hoy" & Chr(34) &
                        ", este activa!"
                Return lblnPuede
            End If
        End If
            ...
            If aenuTipoDoc = EnuTipoDocOri.EnuFactura Then
                ...
                If lblnPuede Then
                    ...
                    Else
                        lblnPuede = Not FblnHayPrefacturas()
                        If Not lblnPuede Then
                            astrMens = "No es posible facturar manualmente si hay " &
    ...
    End Function
    ''' <summary>
    ''' Actualiza el estado del documento identificado por los argumentos "astrPrefijo" y 
    ''' "ashrIdDoc" a "Rechazado" 
    ''' </summary>
    Friend Shared Sub SEstadoRechazado(aenuTipoDoc As EnuTipoDocOri, astrPrefijo As String,
            ashrIdDoc As Short)
        Dim lobjValorLlave As Object() = {GshrIdCarpeta, GshrIdCentroUtil, astrPrefijo,
                ashrIdDoc}
        If aenuTipoDoc = EnuTipoDocOri.EnuFactura Then
            Dim lobjFac As New ClsFactura()
            lobjFac.SAbra(lobjValorLlave)
            If lobjFac.BlnExiste Then
                lobjFac.EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuModificando
                lobjFac.ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuRechazada
                lobjFac.SActualice(True)
            End If
        ElseIf aenuTipoDoc = EnuTipoDocOri.EnuNotaCr Then
            Dim lobjNCr As New ClsNotaCr()
            lobjNCr.SAbra(lobjValorLlave)
            If lobjNCr.BlnExiste Then
                lobjNCr.EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuModificando
                lobjNCr.ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuRechazada
                lobjNCr.SActualice(True)
            End If
        ElseIf aenuTipoDoc = EnuTipoDocOri.EnuNotaDb Then
                Dim lobjNDb As New ClsNotaDb()
                lobjNDb.SAbra(lobjValorLlave)
                If lobjNDb.BlnExiste Then
                    lobjNDb.EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuModificando
                    lobjNDb.ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuRechazada
                    lobjNDb.SActualice(True)
                End If
            End If
    End Sub
    ...
    Friend Shared Function FdblTasaDscto(astrNroFact As String, ashrIdItemFac As Short,
        ...
        Select Case aenuTipoDscto
            Case EnuTipoDescuento.EnuDsctoCapital
                adecBaseDscto = lobjItemFac.DecDeudaSer
            Case EnuTipoDescuento.EnuDsctoIntMora
                adecBaseDscto = lobjItemFac.FdecDeudaIntTotal
            Case EnuTipoDescuento.EnuDsctoPP
                adecBaseDscto = lobjItemFac.DecValorServicio()
            Case EnuTipoDescuento.EnuReteFuente
                adecBaseDscto = lobjItemFac.DecValorServicio()
            Case EnuTipoDescuento.EnuReteIca
                adecBaseDscto = lobjItemFac.DecValorServicio()
            Case EnuTipoDescuento.EnuReteIva
                adecBaseDscto = lobjItemFac.DecIvaServicio
            Case EnuTipoDescuento.EnuCancelaIva
                adecBaseDscto = lobjItemFac.FdecDeudaIvaTotal
        End Select
...
aobjItemRec.ObjIdItemRecCajaShr.ObjValorPro & " no generó Novedad."
    ...
    Private Sub SVerifiqueIntegItemsNCR(aobjItemNCR As ClsItemNotaCr)
        ...
        If lobjNotaCr.ColNovedades.Count > 0 Then
            Dim ldecVlrNovedades = 0D
            Dim ldecVlrNov As Decimal
            Dim lenuTipoItemNCR As EnuTipoDescuento = aobjItemNCR.ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro
            Dim lenuTipoNov As EnuTipoNov
            Dim lblnHayError As Boolean
                ...
                If lobjNov.ObjIdItemDocOrigen_NovShr.ObjValorPro = aobjItemNCR.ObjIdItemNotaCrShr.ObjValorPro Then
                    ...
                    Select Case lenuTipoNov
                        Case EnuTipoNov.EnuCrDctoCap, EnuTipoNov.EnuRDbIva,
                            ...
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuDsctoCapital) AndAlso
                                    (lenuTipoItemNCR <> EnuTipoDescuento.EnuDsctoPP) AndAlso
                                    (lenuTipoItemNCR <> EnuTipoDescuento.EnuDsctoIntMora) AndAlso
                                    (lenuTipoItemNCR <> EnuTipoDescuento.EnuCancelaIva)
                        ...
                        Case EnuTipoNov.EnuRCrDctoCap, EnuTipoNov.EnuRCrIvaGas
                            ...
                            ldecVlrNovedades -= ldecVlrNov
                        Case EnuTipoNov.EnuCrDctoInt
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuDsctoIntMora)
                            ldecVlrNovedades += ldecVlrNov
                        Case EnuTipoNov.EnuRCrDctoInt
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuDsctoIntMora)
                            ldecVlrNovedades -= ldecVlrNov
                        Case EnuTipoNov.EnuCrRetFte
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuReteFuente)
                            ldecVlrNovedades += ldecVlrNov
                        Case EnuTipoNov.EnuRCrRetFte
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuReteFuente)
                            ldecVlrNovedades -= ldecVlrNov
                        Case EnuTipoNov.EnuCrRetIca
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuReteIca)
                            ldecVlrNovedades += ldecVlrNov
                        Case EnuTipoNov.EnuRCrRetIca
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuReteIca)
                            ldecVlrNovedades -= ldecVlrNov
                        Case EnuTipoNov.EnuCrRetIva
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuReteIva)
                            ldecVlrNovedades += ldecVlrNov
                        Case EnuTipoNov.EnuRCrRetIva
                            lblnHayError = (lenuTipoItemNCR <> EnuTipoDescuento.EnuReteIva)
                            ldecVlrNovedades -= ldecVlrNov
    ...
    Friend Shared Function FblnHayPrefacturas() As Boolean
        ...
        If lentCantAProcesar > 0 Then
            ...
            DtmFechaFacturasAReversar = ClsPanorama.FobjValorCampo(
                    ldrwPrefacturas(0)(ClsFechaFacturaDtm.SstrNombreCampoBd),
                    EnuTipoValor.EnuDate)
        End If
        Return lblnHayFac
    End Function
    ...
    Friend Shared Function FblnDocPorProcesarEFac() As Boolean
        If GobjParametros.BlnEFacAutorizado Then
            ...
            Dim lstrFiltro = StrFiltroUbicacion & " AND " &
                    ...
                    " AND " & ClsFechaFacturaDtm.SstrNombreCampoBd & " = " & lstrFechaHoy
            Dim ldtb = ClsPanorama.FdtbDataTable(ClsFactura.SstrNombreTabla, lstrCamposSel,
                    Nothing, lstrFiltro)
            lentCanDoc = ClsPanorama.FobjValorCampo(ldtb.Rows(0)(0), EnuTipoValor.EnuInteger)
            If lentCanDoc > 0 Then
                Return True
            End If
            lstrFiltro = StrFiltroUbicacion & " AND (" & ClsIdEstadoEDocEnt.SstrNombreCampoBd &
                    " = " & EnuEstadoEDoc.EnuRechazada & " OR " &
                    ClsIdEstadoEDocEnt.SstrNombreCampoBd & " = " &
                    EnuEstadoEDoc.EnuOtro & ")"
            ldtb = ClsPanorama.FdtbDataTable(ClsFactura.SstrNombreTabla, lstrCamposSel,
                    Nothing, lstrFiltro)
            lentCanDoc = ClsPanorama.FobjValorCampo(ldtb.Rows(0)(0), EnuTipoValor.EnuInteger)
            If lentCanDoc > 0 Then
                Return True
