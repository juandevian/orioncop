...
#Region "Procedimientos y funciones invalidantes"
    ...
    Friend Overrides Function FblnSonValidosDatosOrigen(adtbOrigen As DataTable,
            ablnReinicie As Boolean, ByRef astrMens As String) As Boolean
        Dim lblnEsValido = False, i = 0
        ...
        For Each ldrwOrigen As DataRow In adtbOrigen.Rows
            i += 1
            lbytIdCar = ClsPanorama.FobjValorCampo(ldrwOrigen("IdCarpeta"), EnuTipoValor.enuString)
            lbytIdCenutil = ClsPanorama.FobjValorCampo(ldrwOrigen("IdCentroUtil"), EnuTipoValor.enuString)
            ldblIdTerCliente = ClsPanorama.FobjValorCampo(ldrwOrigen("IdTerceroCliente"), EnuTipoValor.enuDouble)
            lblnEsValido = ClsPanorama.FblnEsValidoNumero(ldblIdTerCliente, 1, Double.MaxValue, True,
                    EnuTipoValor.enuDouble)
            If Not lblnEsValido Then
                astrMens = "La Identificación del Cliente en el Registro " & i.ToString & " no es válido!"
                Exit For
            End If
...
#Region "Causación Mora"
    ...
    Private Function SCauseMoraPreAgr(astrIdPrediosAgru() As String, adtmFechaCausacion As Date,
        ...
        For Each lobjFac As ClsFactura In lcolFrasVivas
            If lobjFac.FdecIvaServicios > 0 Then
...
#Region "Deuda del Cliente"
    ...
    ''' para el predio agrupador "astrIdPredioAgru" y el agrupador de servicios 
    ''' "ashrIdAgruServicios"
    ''' </summary>
    ''' <param name="astrIdPredioAgru">Id del Predio Agrupador al cual pertenece las facturas.</param>
    ''' <param name="ashrIdAgruServicios">Identifica al agrupador de servicios de los Servicios 
    ''' incluidos en la factura. Si este parametro es cero se devuelve la información de todas las facturas con saldo.</param>
    ''' <returns></returns>
    ''' <remarks></remarks>
    ...
    Friend Function FstrIdFacturasConMora(astrIdPrediosAgru As String()) As ArrayList 'Ok
        ...
        For Each lobjFactura As ClsFactura In lcolFrasVivas
            If lobjFactura.FdecDeudaIntMora > 0 Then
    ...
    Friend Function FstrIdFacturasVivas(astrIdPrediosAgru As String(),
        ...
        For Each lobjFactura As ClsFactura In lcolFrasVivas
            If lobjFactura.DecDeudaCapitalSer(astrServicios) > 0 Then
                lstrIdFactVivas.Add(lobjFactura.StrNumeroFactura)
            End If
        Next
        Return lstrIdFactVivas
    ...
    Friend Function FdtbDetalleDeuda(astrIdPrediosAgru As String(),
        ...
        For Each lobjFact As ClsFactura In lcolFrasVivas
            ...
            ldecDeudaCap = lobjFact.DecDeudaCapitalSer(astrServicios)
    ...
    Private Sub SComplementeDtbEstado(adtbItemsFacEstado As DataTable, astrSer As String)
        ...
        Dim lentIdFacs As New List(Of String)
        ...
        For Each ldrwTbl As DataRow In adtbItemsFacEstado.Rows
            ...
            If Not lentIdFacs.Contains(lstrIdFact) Then
                lentIdFacs.Add(lstrIdFact)
                lobjFac.SAbra({GshrIdCarpeta, GshrIdCentroUtil, lstrPref, lentIdFac})
                ldecVlrItem = lobjFac.DecDeudaServicio(astrSer)
                ldecDeuCap = lobjFac.DecDeudaCapitalSer({astrSer})
                ldecDeuMora = lobjFac.DecDeudaIntMoraSer({astrSer})
...
#Region "Retenciones y Descuentos"
    ...
    Private Sub SPuebleDtbDescuentos(aobjItemFactura As ClsItemFactura, adtmFechaPago As Date,
        ...
        If ldecDsctoProntoPago > 0 AndAlso aobjItemFactura.FdecDeudaServicioTotal > ldecTotalDscto Then
            ...
            ldrwNuevoDscto("IdTipoDcto") = EnuTipoDescuentoDef.EnuDsctoPP
            ...
            lstrNombreDcto = ClsOrionCop.FstrNombreDatoConstanteOri(
                    EnuGrupoConstantesOriDef.EnuTipoDescuento, EnuTipoDescuentoDef.EnuDsctoPP)
            ldrwNuevoDscto("TipoDcto") = lstrNombreDcto
            adtbDsctos.Rows.Add(ldrwNuevoDscto)
        End If
        For i = EnuTipoDescuentoDef.EnuReteFuente To EnuTipoDescuentoDef.EnuReteCree
                ...
                If MobjPadre.EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
                    ...
                    If HblnEsValido Then
                        ...
                        If MobjPadre.ObjTerceroCliente.EnuEstadoActualizacion =
                            ...
                            If lblnExiste Then
                                MobjPadre.ObjTerceroCliente.SAbra({HobjValorNew})
                                MobjPadre.ObjTerceroCliente.SModifique()
                            Else
