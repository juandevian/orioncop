...
#Region "Procedimientos del objeto"
    ...
    Friend Sub SModifiqueADefinitiva()
        ...
        Dim lcolItemsFactura = ColItemsFactura
        ...
        For Each lobjItemFactura As ClsItemFactura In lcolItemsFactura
    ...
    Friend Function FblnPredioAgrupadorEsValido(astrIdPredioAgrupador As String) As Boolean
        ...
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
            If Not IsNothing(ColItemsFactura) AndAlso ColItemsFactura.Count > 0 Then
    ...
    Friend Function FbytQRFact() As Byte()
        ...
        Dim lstrFac As String = "NumFac:" & StrNumeroFactura & vbCrLf &
                ...
                "ValFac:" & Format(DecValorServiciosFact, "#0.00") & vbCrLf &
                "ValIva:" & Format(DecIvaFactura, "#0.00") & vbCrLf &
    ...
    Friend Function FdtbFactsUltimoMes() As DataTable
        Dim lstrTablaPri = SstrNombreTabla
        Dim lstrTablaSec = ClsCliente.SstrNombreTabla
        Dim lstrCampSelPri As String() = {ClsFechaFacturaDtm.SstrNombreCampoBd,
                ClsPrefijo_FactStr.SstrNombreCampoBd, ClsIdFacturaEnt.SstrNombreCampoBd,
                ClsIdPredioAgrupador_FacStr.SstrNombreCampoBd,
                ClsIdCliente_FactDbl.SstrNombreCampoBd, ClsValor_FactDec.SstrNombreCampoBd}
        ...
        Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion_Pri & " AND " &
                ClsFechaFacturaDtm.SstrNombreCampoBd & " BETWEEN " & lstrFechaIni & " AND " &
                lstrFechaFin
        Dim lstrOrden As String(,) = {{ClsFechaFacturaDtm.SstrNombreCampoBd, "DESC"},
                {ClsPrefijo_FactStr.SstrNombreCampoBd, "ASC"},
                {ClsIdFacturaEnt.SstrNombreCampoBd, "DESC"}}
        Dim ldtbFacturas = ClsPanorama.FdtbDataTable(lstrTablaPri, lstrCampSelPri, lstrTablaSec,
                lstrCampSelSec, lstrCampRelPri, lstrCampRelSec, lstrOrden, True, lstrFiltro, {})
        Return ldtbFacturas
    End Function
#End Region

#Region "Informacion para la anulación de la factura"
    Friend Function FdecIvaTotal() As Decimal
        Return FdecIvaCapital() + FdecIvaIntSer()
    End Function

    Friend Function FdecIvaCapital() As Decimal
        Dim ldecDeudaIva = 0D
        For Each lobjItemFact As ClsItemFactura In ColItemsFactura
            ldecDeudaIva += lobjItemFact.DecIvaServicio
        Next
        Return ldecDeudaIva
    End Function
    ...
    Friend Function FdecIvaIntSer() As Decimal
        Dim ldecIvaInt = 00D
        For Each lobjItemFact As ClsItemFactura In ColItemsFactura
            ldecIvaInt += lobjItemFact.FdecIvaIntSer
        Next
        Return ldecIvaInt
    End Function
    Friend Function FdecBaseIvaTotal() As Decimal
        Return DecBaseIvaFactura() + FdecBaseIvaIntSer()
    End Function

    Friend Function FdecBaseIvaIntSer() As Decimal
        Dim ldecBaseIva = 0D
        For Each lobjItemFactura As ClsItemFactura In ColItemsFactura
            If lobjItemFactura.ObjTarifaIva_ItemFactDbl.ObjValorPro > 0 Then
                ldecBaseIva += lobjItemFactura.FdecBaseIvaIntSer
            End If
        Next
        Return ldecBaseIva
    End Function

    Friend Sub SHabiliteAnular()
        If HenuTipoObjeto = EnuModoInstanciaObjDef.EnuUnico Then
            HblnEsAnulable = True
            HblnEsAnulable = FblnEsAnulable()
        End If
    End Sub
#End Region
...
#Region "eFac"
    ...
    Friend Sub SCambieANoFacEle()
        If ((ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuErrorFtp OrElse
                ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuInvalida) AndAlso
                ObjFechaFacturaDtm.ObjValorPro < Date.Today) Then
            If EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuConsultando Then
                EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuModificando
            End If
            ObjIdEstadoEDocEnt.ObjValorPro = EnuEstadoEDoc.EnuNoEDoc
...
#Region "Manejo Items Factura"
    ...
    Friend Sub SAdicioneNuevoItem(aobjNuevoItemFactura As ClsItemFactura)
        Try
            Dim lshrIdItemFac As Short = ColItemsFactura.Count + 1
            ...
            ColItemsFactura.Add(aobjNuevoItemFactura,
                    aobjNuevoItemFactura.ObjIdItemFacturaShr.ToString)
    ...
    Friend Sub SAdicioneItemSerId(aobjNuevoItemFactura As ClsItemFactura)
        Try
            With aobjNuevoItemFactura
                ...
                .ObjEsExcluidoIva_ItemFactBln.ObjValorPro = .ObjServicio.ObjEsExcluidoIvaBln.ObjValorPro
                .ObjTarifaIva_ItemFactDbl.ObjValorPro = .ObjServicio.ObjTarifaIvaDbl.ObjValorPro
                .ObjIdItemFacturaShr.ObjValorPro = ColItemsFactura.Count + 1
    ...
    Friend Sub SElimineItem(ashrIdItemFra As Short)
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuCreando Then
            If ColItemsFactura.Count > 0 AndAlso ColItemsFactura.Count >= ashrIdItemFra Then
                ColItemsFactura.Remove(ashrIdItemFra)
                SRenumereItems()
                SCalculeValorFactura()
            Else
    ...
    Private Sub SCargueDtbItems()
        If IsNothing(MdtbItemsFact) OrElse MdtbItemsFact.Rows.Count = 0 Then
            Dim lstrIdFactura = "0"
            If EnuEstadoActualizacion <> EnuEstadoObjetoDef.EnuCreando Then
            ...
            Dim lstrFiltro = ClsOrionCop.StrFiltroUbicacion & " AND " &
                    ...
                    ObjPrefijo_FactStr.ToString & "' AND " &
                    ClsIdFactura_ItemFactEnt.SstrNombreCampoBd & " = " & lstrIdFactura
            Dim lstrCamposSelect() = {"*", "0 as VlrIva"}
            MdtbItemsFact = ClsPanorama.FdtbDataTable(ClsItemFactura.SstrNombreTabla,
    ...
    Private Sub SComplementeTablaItems()
        If MstrNroFactItem <> StrNumeroFactura Then
            ...
            For Each ldrwItFa As DataRow In ldrwItemsFactura
                ...
                If ColItemsFactura.Contains(lshrIdItemFac.ToString) Then
                    lobjItemFac = ColItemsFactura(lshrIdItemFac.ToString)
                End If
                ldecVlrIvaItem = lobjItemFac.DecIvaServicio
    ...
    Friend Function FblnFacturoServicio(ashrIdAno As Short, ashrIdServicio As Short,
            ablnEsSerAjuste As Boolean)
        Dim lblnFactSer = False
        If ashrIdAno > 0 Then
            If ablnEsSerAjuste Then
                lblnFactSer = True
            Else
                For Each lobjItemFac As ClsItemFactura In ColItemsFactura
                    lblnFactSer = lobjItemFac.ObjIdServicio_ItemFactShr.ObjValorPro = ashrIdServicio
                    If lblnFactSer Then Exit For
                Next
            End If
        Else
            For Each lobjItemFac As ClsItemFactura In ColItemsFactura
                If lobjItemFac.ObjIdAno_ServicioItemFactShr.ObjValorPro =
                    ashrIdAno Then
                    lblnFactSer = lobjItemFac.ObjIdServicio_ItemFactShr.ObjValorPro = ashrIdServicio
                    If lblnFactSer Then Exit For
                End If
...
#Region "Iva y Retenciones"
    Friend Function FstrIvasFactura() As String()
        ...
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            If lobjItemFac.DecIvaServicio > 0 Then
    ...
    Private Function FstrValoresIva(adblTasaIva As Double) As String
        ...
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            If lobjItemFac.ObjTarifaIva_ItemFactDbl.ObjValorPro = adblTasaIva Then
                ldecTotaBase += lobjItemFac.DecBaseIvaServicio
                ldecTotaVlrIva += lobjItemFac.DecIvaServicio
            End If
        Next
...
#Region "Calculo y causación intereses mora"
        ...
        Get
            ...
            If lblnCauseMora Then
                If FdecDeudaCapital > 0 Then
                    For Each lobjItemFactura As ClsItemFactura In ColItemsFactura
                        If lobjItemFactura.FdecDeudaCapital > 0 Then
    ...
    Friend Function SCauseMora(adtmFechaCausacion As Date) As Decimal
        ...
        For Each lobjItemFact As ClsItemFactura In ColItemsFactura
            ...
            StcIntMora_DecBase(i) = lobjItemFact.FdecDeudaCapital
...
#Region "Aplica Creditos (Pagos, Descuentos, Retenciones y Anticipos)"
    Friend Function FblnEsValidoDescuento(ashrIdItemFac As Short, adecValorDscto As Decimal,
            aenuTipoDescuento As EnuTipoDescuento, ByRef astrMens As String,
            ByRef aenuSevNot As EnuSeveridadNot) As Boolean
        Dim lblnEsValido = True, ldecBase = 0D, ldblTasa = 0.0
        Dim lobjItemFac As ClsItemFactura = ColItemsFactura(ashrIdItemFac.ToString)
        If aenuTipoDescuento >= EnuTipoDescuento.EnuReteFuente AndAlso aenuTipoDescuento <=
        ...
        If lblnEsValido Then
            Select Case aenuTipoDescuento
                ...
                Case EnuTipoDescuento.EnuReteIva
                    lblnEsValido = (adecValorDscto < lobjItemFac.FdecDeudaCapital)
                    If lblnEsValido Then
                        If lobjItemFac.FdecValorRetencion(aenuTipoDescuento, ldecBase, ldblTasa) <>
                                        adecValorDscto Then
                            astrMens = "El valor de la Retención ingresado no concuerda con los " &
                ...
                Case EnuTipoDescuento.EnuCancelaIva
                    lblnEsValido = adecValorDscto <= lobjItemFac.FdecDeudaIvaTotal
                    If Not lblnEsValido Then
    ...
    Friend Sub SApliqueCreditoRC(aobjItemRec As ClsItemRecCaja)
        ...
        If lblnACapital Then
            Dim lblnEsDsctoPP = aobjItemRec.ObjIdTipoItemRecByt.ObjValorPro =
                    EnuTipoItemRecCajaDef.EnuDsctoPP
    ...
    Private Sub SApliqueCrRecIntMora(aobjItemRec As ClsItemRecCaja) 'Ok
        ...
        For Each lobjItemFra As ClsItemFactura In ColItemsFactura
            ldecVlrAplicar = lobjItemFra.FdecDeudaIntTotal
    ...
    Private Sub SApliqueCrRecCap(aobjItemRec As ClsItemRecCaja)
        ...
        ldecVlrAplicar = lobjItemFra.FdecDeudaCapital
        If ldecVlrAplicar > 0 Then
        ...
        SVerifiqueCancelacion(ldtmFechaCr)
    End Sub
    Private Sub SApliqueDsctoPP(aobjItemRec As ClsItemRecCaja) 'Ok
        Dim lenuTipoNov As EnuTipoNov = aobjItemRec.FenuTipoNovedad()
        Dim lobjRecCaja As ClsReciboCaja = aobjItemRec.ObjPadre
        Dim ldtmFechaCr As Date = lobjRecCaja.ObjFechaRecDtm.ObjValorPro
        Dim lstrIdCtaDb As String = aobjItemRec.ObjIdCuentaDb_ItemRecStr.ObjValorPro
    ...
    Friend Sub SApliqueNotaCr(aobjItemNotaCr As ClsItemNotaCr)
        ...
        If lenuTipoDscto = EnuTipoDescuento.EnuDsctoIntMora Then
            If ldecVlrCr > lobjItemFac.DecDeudaInt Then
                Throw New ErrorInesperadoPanLException("Valor Item NotaCr mayor a la deuda por Mora")
            End If
        ElseIf lenuTipoDscto = EnuTipoDescuento.EnuDsctoCapital Then
            If ldecVlrCr > (lobjItemFac.DecDeudaSer) Then
        ...
        ElseIf lenuTipoDscto = EnuTipoDescuento.EnuCancelaIva Then
            If ldecVlrCr > lobjItemFac.FdecDeudaIvaTotal Then
                Throw New ErrorInesperadoPanLException("Valor Item NotaCr mayor a la deuda por Mora")
            End If
        End If
        If EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuConsultando Then
...
#Region "Valores facturados"
    ''' <summary>
    ''' Devuelve el valor total de los servicios facturados sin incluir el IVA
    ''' </summary>
    ''' <returns></returns>
    Friend ReadOnly Property DecValorServiciosFact As Decimal
        Get
            Dim ldecVlr = 0D
            For Each lobjItemFac As ClsItemFactura In ColItemsFactura
                ldecVlr += lobjItemFac.DecValorServicio()
            Next
            Return ldecVlr
        End Get
    End Property

    Friend ReadOnly Property DecValorIntFactura As Decimal
        Get
            Dim ldecValorInt = 0D
            For Each lobjItemFac As ClsItemFactura In ColItemsFactura
                ldecValorInt += lobjItemFac.DecValorIntereses
            Next
            Return ldecValorInt
        End Get
    End Property
    ...
    ''' <summary>
    ''' devuelve la base del IVA total de los servicios facturados
    ''' </summary>
    ''' <returns></returns>
    Friend ReadOnly Property DecBaseIvaFactura() As Decimal
        Get
            Dim ldecDeuda = 0D
            For Each lobjItemFact As ClsItemFactura In ColItemsFactura
                ldecDeuda += lobjItemFact.DecBaseIvaServicio()
            Next
            Return ldecDeuda
        End Get
    End Property
    ...
    ''' <summary>
    ''' Devuelve el valor del IVA total de los servicios facturados
    ''' </summary>
    ''' <returns></returns>
    Friend ReadOnly Property DecIvaFactura() As Decimal
        Get
            Dim ldecValorIva = 0D
            For Each lobjItemFact As ClsItemFactura In ColItemsFactura
                ldecValorIva += lobjItemFact.DecIvaServicio
            Next
            Return ldecValorIva
        End Get
    End Property
    ...
    ''' <summary>
    ''' Devuelve la base del IVA generado por intereses de la factura
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecBaseIvaIntFac() As Decimal
        Dim ldecBase = 0D
        For Each lobjItemFact As ClsItemFactura In ColItemsFactura
            ldecBase += lobjItemFact.FdecBaseIvaIntSer
        Next
        Return ldecBase
    End Function

    ''' <summary>
    ''' Devuelve el valor del IVA generado por intereses de la factura
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecIvaIntFac() As Decimal
        Dim ldecIvaInt = 0D
        For Each lobjItemFact As ClsItemFactura In ColItemsFactura
            ldecIvaInt += lobjItemFact.FdecIvaIntSer
        Next
    ...
    '''Devuelve el valor del capital facturado de los servicios incluidos en el arreglo de 
    ''' servicios "astrServicios"
    ''' </summary>
    ''' 
    Friend Function FdecCapitalServiciosFact(astrServicios As String()) As Decimal
        Dim ldecVlrFact = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            If astrServicios.Contains("A") Then
                ldecVlrFact += lobjItemFac.ObjValor_ItemFactDec.ObjValorPro
            Else
                If lobjItemFac.ObjIdAno_ServicioItemFactShr.ObjValorPro > 0 Then
                    ...
                    End If
                Else
                    If astrServicios.Contains(
                            lobjItemFac.ObjIdServicio_ItemFactShr.ToString()) Then
                        ldecVlrFact += lobjItemFac.ObjValor_ItemFactDec.ObjValorPro
                    End If
                End If
            End If
...
#Region "Información deuda"
    ...
    ' Deuda de capital
    ''' <summary>
    ''' Devuelve el valor total de la deuda de la factura incluyendo el IVA
    ''' </summary>
    Friend ReadOnly Property FdecDeudaCapital As Decimal
        Get
            Dim ldecDeudaCapital = 0
            For Each lobjItemFac As ClsItemFactura In ColItemsFactura
                ldecDeudaCapital += lobjItemFac.FdecDeudaCapital
            Next
    ...
    ' Deuda servicios
    ''' <summary>
    ''' Devuelve el valor total de la deuda por servicios sin incluir el IVA
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecDeudaServicios() As Decimal
        Dim ldecDeuda = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            ldecDeuda += lobjItemFac.DecDeudaSer
        Next
        Return ldecDeuda
    End Function

    ''' <summary>
    ''' Devuelve la deuda de capital de los servicios incluidos en el arreglo de servicios
    ''' "astrServicios"
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecDeudaCapitalSer(astrServicios As String()) As Decimal
        Dim ldecDeuda = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            If astrServicios.Contains("A") Then
                ldecDeuda += lobjItemFac.FdecDeudaCapital
            Else
                If lobjItemFac.ObjIdAno_ServicioItemFactShr.ObjValorPro > 0 Then
                    If astrServicios.Contains("0") Then
                        ldecDeuda += lobjItemFac.FdecDeudaCapital
    ...
    ' Deuda Int Mora
    ''' <summary>
    ''' Devuelve el valor total de la deuda por intereses de mora sin incluyir el IVA
    ''' </summary>
    Friend Function FdecDeudaIntServicios() As Decimal
        Dim ldecDeuda = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            ldecDeuda += lobjItemFac.DecDeudaInt
        Next
        Return ldecDeuda
    End Function
    ...
    Friend Function FdecDeudaIntTotal() As Decimal
        Dim ldecDeuda = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            ldecDeuda += lobjItemFac.FdecDeudaIntTotal
        Next
        Return ldecDeuda
    End Function

    ''' <summary>
    ''' Devuelve la deuda por intereses de mora de los servicios incluidos en el arreglo 
    ''' de servicios "astrServicios"
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecDeudaIntMoraSer(astrServicios As String()) As Decimal
        Dim ldecDeuda = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            If astrServicios.Contains("A") Then
                ldecDeuda += lobjItemFac.FdecDeudaIntTotal
    ...
    ''' Deuda Iva total
    ''' <summary>
    ''' Devuelve el valor total de la deuda por IVA de la factura
    ''' </summary>
    Friend Function FdecDeudaIvaTotal() As Decimal
        Dim ldecDeuda = 0D
        For Each lobjItemFac As ClsItemFactura In ColItemsFactura
            ldecDeuda += lobjItemFac.FdecDeudaIvaTotal
        Next
        Return ldecDeuda
    End Function
#End Region
    ...
    Public Overrides Sub SValide()
        ...
        If HblnEsValido Then
            If MobjPadre.EnuEstadoActualizacion = EnuEstadoObjetoDef.EnuCreando Then
                ...
                Else
                    If GobjParametros.BlnEFacAutorizado Then
                        If String.IsNullOrEmpty(MobjCliente.ObjEmailStr.ToString) Then
                            HblnEsValido = False
                            HstrMens = "El Cliente seleccionato no tiene un Email asignado!"
                        End If
                    End If
                End If
