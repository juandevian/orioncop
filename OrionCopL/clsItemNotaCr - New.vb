...
#Region "Otras propiedades"
    ...
    Friend ReadOnly Property ObjItemFac As ClsItemFactura
        ...
        End Get
    End Property
    Friend ReadOnly Property DblTasaDsctoServicio(aenuTipodscto As EnuTipoDescuentoDef) As Double
        Get
            Dim ldblTasadscto = 0.0
            If aenuTipodscto > EnuTipoDescuentoDef.EnuReteFuente Then
    ...
    Friend ReadOnly Property BlnEsDscto As Boolean
        Get
            Dim lenuTipoDscto As EnuTipoDescuentoDef = ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro
            Dim lblnEsDscto As Boolean = (lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoCapital OrElse
                    lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoIntMora OrElse
                    lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoPP)
            Return lblnEsDscto
        End Get
    End Property
    Friend ReadOnly Property BlnEsRetencion As Boolean
        Get
            Dim lenuTipoDscto As EnuTipoDescuentoDef = ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro
            Dim lblnEsRet As Boolean = Not (lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoCapital OrElse
                    lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoIntMora OrElse
                    lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoPP)
            Return lblnEsRet
        End Get
...
#Region "Procedimientos y funciones invalidantes"
    ...
    Protected Overrides Sub SInicialiceObj()
        ...
        ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro = EnuTipoDescuentoDef.None
...
#Region "Procedimientos del objeto"
    Friend Function FblnEsValidaFechaNotaCr(adtmFechaNotaCr As Date) As Boolean
        ...
        If ObjPrefijoFact_ItemNotaCrStr.BlnEsValido AndAlso
            ...
            If MobjPadre.ObjIdPredioAgrupador_NotaCrStr.BlnEsValido Then
                ...
                Dim lstrIdPredAgr = {MobjPadre.ObjIdPredioAgrupador_NotaCrStr.ToString()}
                Dim lcolFrasVivas As Collection = lobjCliente.FcolFacturas(lstrIdPredAgr, {"A"}, True)
                If lcolFrasVivas.Contains(StrNroFactura_ItemNotaCr) Then
                    Dim lobjFactura As ClsFactura = lcolFrasVivas(StrNroFactura_ItemNotaCr)
                    lblnEsValida = (adtmFechaNotaCr >= lobjFactura.ObjFechaFacturaDtm.ObjValorPro)
                End If
            End If
        End If
        Return lblnEsValida
    ...
    Friend Function FblnEsValidoDscto(adecValorDscto As Decimal,
        ...
        If lblnEsValido Then
            ...
            Dim lenuTipoDscto As EnuTipoDescuentoDef = ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro
            If lenuTipoDscto >= EnuTipoDescuentoDef.EnuReteFuente AndAlso lenuTipoDscto <=
            ...
            End If
            If lblnEsValido Then
                If lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoCapital Then
                    Dim ldecDeudaSinIva = lobjItemFac.FdecDeudaCapital -
                            lobjItemFac.FdecDeudaIvaCapital
                    lblnEsValido = adecValorDscto <= (ldecDeudaSinIva)
                    If Not lblnEsValido Then
                        astrMens = "El Valor ingresado es mayor a la Deuda de Capital!"
                    End If
                ElseIf lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoIntMora Then
                    lblnEsValido = adecValorDscto <= ObjItemFac.FdecDeudaIntMora
                    If Not lblnEsValido Then
        ...
        Return lblnEsValido
    End Function
    Friend Function FblnEsValidoItemDscto(aenuIdTipoDscto As EnuTipoDescuentoDef,
                ByRef astrMens As String) As Boolean
        GobjPanDat.SControleProcesoObj(True)
        Dim lobjNotaCr As ClsNotaCr = ObjPadre
        Dim lblnEsValido = Not lobjNotaCr.FblnExisteNuevoItem(
    ...
    Friend Function FblnEsDscto() As Boolean
        Dim lenuTipoItem As EnuTipoDescuentoDef = ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro
        Return (lenuTipoItem = EnuTipoDescuentoDef.enuDsctoCapital) OrElse
                (lenuTipoItem = EnuTipoDescuentoDef.enuDsctoIntMora) OrElse
                (lenuTipoItem = EnuTipoDescuentoDef.enuDsctoPP)
    End Function
    Friend Function FblnEsRetencion() As Boolean
        Dim lenuTipoItem As EnuTipoDescuentoDef = ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro
        Return (lenuTipoItem > EnuTipoDescuentoDef.EnuDsctoIntMora) AndAlso
                (lenuTipoItem < EnuTipoDescuentoDef.EnuDsctoPP)
    End Function
#End Region
End Class
    ...
    Public Overrides Sub SValide()
        ...
        If HblnEsValido Then
            If MobjPadre.EnuEstadoActualizacion = EnuEstadoObjetoDef.enuCreando Then
                If MobjPadre.ObjIdTipoDscto_ItemNotaCrByt.ObjValorPro >= EnuTipoDescuentoDef.EnuDsctoIntMora Then
...
Friend Class ClsIdTipoDscto_ItemNotaCrByt
    ...
    Public Overrides Sub SValide()
        HblnEsValido = ClsPanorama.FblnEsValidoEnumByte(HobjValorNew,
                EnuTipoDescuentoDef.EnuDsctoCapital, EnuTipoDescuentoDef.EnuCancelaIva,
                BlnEsRequerido)
        If HblnEsValido Then
            If MobjPadre.EnuEstadoActualizacion <> EnuEstadoObjetoDef.enuCreando Then
                HblnEsValido = (HobjValorNew = HobjValorOriginal)
            Else
                Dim lenuTipoDscto As EnuTipoDescuentoDef = HobjValorNew
                HblnEsValido = MobjPadre.FblnEsValidoItemDscto(lenuTipoDscto, HstrMens)
                If HblnEsValido AndAlso lenuTipoDscto = EnuTipoDescuentoDef.EnuDsctoPP Then
