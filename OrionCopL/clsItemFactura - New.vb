...
#Region "Deuda"
    ...
    ' Deuda capital
    ''' <summary>
    ''' Devuelve el valor de la deuda de capital incluido el IVA al momento de la última 
    ''' lectura de las novedades
    ''' </summary>
    Friend Function FdecDeudaCapital() As Decimal
        Dim ldecDeudaCapital = FdecDeudaCapitalTotal() - MdecCrAplicadoCap
        Return ldecDeudaCapital
    End Function
    ''' <summary>
    ''' Devuelve la deuda de capital incluido el IVA
    ''' </summary>
    ''' <returns></returns>
    Private Function FdecDeudaCapitalTotal() As Decimal
    ...
    End Function
    ''' <summary>
    ''' Devuelve el total de los intereses debidos sin incluir el Iva a ellos,
    ...
    Friend Function FdecDeudaIva() As Decimal
        Dim ldecDeudaCapital = FdecDeudaCapitalTotal()
        Dim ldecIvaGenerado = FdecIvaGeneradoCap() + FdecIvaGeneradoInt()
        Dim ldecIvaAlGasto = FdecIvaAlGasto()
        Dim ldecDeudaIva As Decimal
        If ldecDeudaCapital < ldecIvaGenerado - ldecIvaAlGasto Then
            ldecDeudaIva = ldecDeudaCapital
        Else
            ldecDeudaIva = ldecIvaGenerado - ldecIvaAlGasto
        End If
        Return ldecDeudaIva
    End Function
    ''' <summary>
    ''' Devuleve el valor del IVA generado al capital
    ''' </summary>
    ''' <returns></returns>
    Private Function FdecIvaGeneradoCap() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            .Append("SELECT SUM(IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuDbIva).Append(", Valor, 0) - IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRDbIva).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
        ...
        Dim ldecIvaGeneradoCap = ClsPanorama.FobjValorCampo(ldtbRes.Rows(0)("Saldo"),
                EnuTipoValor.EnuDecimal)
        Return ldecIvaGeneradoCap
    End Function
    ''' <summary>
    ''' Devuleve el valor del IVA generado a los intereses de mora
    ''' </summary>
    ''' <returns></returns>
    Private Function FdecIvaGeneradoInt() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
    ...
    End Function
    Private Function FdecIvaAlGasto() As Decimal
        Dim lstbSql As New StringBuilder
        Dim lstrPref = ObjPrefijo_ItemFactStr.ObjValorPro
        Dim lentIdFac = ObjIdFactura_ItemFactEnt.ObjValorPro
        Dim lshrIdItem As Short = ObjIdItemFacturaShr.ObjValorPro
        With lstbSql
            .Append("SELECT SUM(IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuCrIvaGas).Append(", Valor, 0) - IF(")
            .Append(ClsIdTipoNovedadByt.SstrNombreCampoBd).Append(" = ")
            .Append(EnuTipoNov.EnuRCrIvaGas).Append(", Valor, 0)) As Saldo FROM ")
            .Append(ClsNovedad.SstrNombreTabla).Append(" WHERE ")
            .Append(ClsIdCarpetaShr.SstrNombreCampoBd).Append(" = ")
            .Append(GshrIdCarpeta).Append(" AND ").Append(ClsIdCentroUtilShr.SstrNombreCampoBd)
            .Append(" = ").Append(GshrIdCentroUtil).Append(" AND ")
        ...
        Dim lstrSql = lstbSql.ToString
        Dim ldtbRes = ClsPanorama.FdtbDataTable(lstrSql)
        Dim ldecIvaAlGasto = ClsPanorama.FobjValorCampo(ldtbRes.Rows(0)("Saldo"),
                EnuTipoValor.EnuDecimal)
        Return ldecIvaAlGasto
    End Function




    Friend Function FdecDeudaIvaCapital() As Decimal
        Dim ldecDeudaIva = 0D, ldecPagoCap = 0D, lenuTipoNov As EnuTipoNov
            ...
            For Each lobjNov As ClsNovedad In ColNovedades
                ...
                Select Case lenuTipoNov
                    Case EnuTipoNov.EnuDbIva
                        ldecDeudaIva += lobjNov.ObjValor_NovDec.ObjValorPro
                    Case EnuTipoNov.EnuCrAnApCap, EnuTipoNov.EnuCrPagoCap
                        ldecPagoCap += lobjNov.ObjValor_NovDec.ObjValorPro
                    Case EnuTipoNov.EnuRCrAnApCap, EnuTipoNov.EnuRCrPagoCap
                        ldecPagoCap -= lobjNov.ObjValor_NovDec.ObjValorPro
                    Case EnuTipoNov.EnuCrRetIva, EnuTipoNov.EnuCrIvaGas
                        ldecDeudaIva -= lobjNov.ObjValor_NovDec.ObjValorPro
                    Case EnuTipoNov.EnuRCrRetIva, EnuTipoNov.EnuRCrIvaGas
                        ldecDeudaIva += lobjNov.ObjValor_NovDec.ObjValorPro
                End Select
            Next
            If ldecPagoCap >= ldecDeudaIva Then
                ldecDeudaIva = 0
            Else
                ldecDeudaIva -= ldecPagoCap
            End If
        End If
        Return ldecDeudaIva
    End Function
...
#Region "Manejo Novedades"
    ...
    Private Sub SGenereNovedad_CxC_Iva()
        ...
        Dim lobjNovedad As ClsNovedad = FobjNuevaNovedad()
        With lobjNovedad
            .ObjBaseDec.ObjValorPro = FdecBaseIva()
            ...
            .ObjValor_NovDec.ObjValorPro = FdecIva()
        End With
        McolNovedades.Add(lobjNovedad)
    End Sub
    ''' <summary>
...
#Region "Valores del servicio"
    ...
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecBaseIva() As Decimal
        Dim ldecBaseIva = 0D
        Dim ldecValor As Decimal = ObjValor_ItemFactDec.ObjValorPro
        Dim ldblTasaIva As Double = ObjTarifaIva_ItemFactDbl.ObjValorPro
        If ldblTasaIva > 0 Then
    ...
    ''' </summary>
    ''' <returns></returns>
    Friend Function FdecIva() As Decimal
